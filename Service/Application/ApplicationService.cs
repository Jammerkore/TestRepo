...
namespace MIDRetail.Service.Application
{
...
// (CSMITH) - END MID Track #3369
		...
		static void Main()
		{
			...
			//
			ServicesToRun = new System.ServiceProcess.ServiceBase[] { new ApplicationService() };
		...
		private void InitializeComponent()
		{
			...
			// 
			this.AutoLog = false;
			this.ServiceName = "MIDApplicationService";
		...
		protected override void OnStart(string[] args)
		{
			...
			// Register to Application EventLog
			...
			{
				EventLog.WriteEntry("MIDApplicationService", "Error configuring Remoting - " + e.Message, EventLogEntryType.Error);
				return;
			}

			//Begin Fix - JScott - Correct Group Dynamite Conneciton problem
			//// Register ApplicationServiceSession

			//try
			//{
			//    RemotingConfiguration.RegisterActivatedServiceType(typeof(ApplicationServerSession));
			//}
			//catch (Exception e)
			//{
			//    EventLog.WriteEntry("MIDApplicationService", "Error registering ApplicationServiceSession - " + e.Message, EventLogEntryType.Error);
			//    return;
			//}
...
// (CSMITH) - BEG MID Track #3369: DB Connection String hardcoded in config files
			...
			{
				EventLog.WriteEntry("MIDApplicationService", "Database Connection String MUST be specified", EventLogEntryType.Error);
...
// (CSMITH) - END MID Track #3369
            ...
            ApplicationServerGlobal.Load(false);
            // End TT#189  
...
//					_channel = new TcpChannel(Convert.ToInt16(_port));
				...
				{
					EventLog.WriteEntry("MIDApplicationService", "Error opening port #" + _port + " - " + e.Message, EventLogEntryType.Error);
					return;
				}
			}
			catch (Exception e)
			{
				EventLog.WriteEntry("MIDApplicationService", "Error getting port from MyPort on config file - " + e.Message, EventLogEntryType.Error);
				return;
			}
			...
			//try
			//{
			//    _pingObjRef = RemotingServices.Marshal(new ServerPing(eServerType.Application), "MRSServerPing.rem");
			//}
			//catch (Exception e)
			//{
			//    EventLog.WriteEntry("MIDApplicationService", "Error creating or marshalling MRSServerPing - " + e.Message, EventLogEntryType.Error);
			//    return;
			//}
			...
			try
			{
				_sofObjRef = RemotingServices.Marshal(new ServerObjectFactory(eServerType.Application), "MRSServerObjectFactory.rem");
			}
			catch (Exception e)
			{
				EventLog.WriteEntry("MIDApplicationService", "Error creating or marshalling MRSServerObjectFactory - " + e.Message, EventLogEntryType.Error);
				return;
			}
			//End Fix - JScott - Correct Group Dynamite Conneciton problem
			...
			EventLog.WriteEntry("MIDApplicationService", "MIDApplicationService started successfully and listening on port #" + _port, EventLogEntryType.Information);
		}
		...
		protected override void OnStop()
		{
			...
			if (_gcCollectThread != null)
			{
				...
				{
					EventLog.WriteEntry("MIDApplicationService", "Error stopping Garbage Collection thread - " + e.Message, EventLogEntryType.Error);
					return;
				}
			}

            // Begin TT#2307 - JSmith - Incorrect Stock Values
            ApplicationServerGlobal.MessageProcessor.StopMessageListener();
            // End TT#2307
			...
			{
				EventLog.WriteEntry("MIDApplicationService", "Error cleaning up global area - " + e.Message, EventLogEntryType.Error);
				return;
			}
			...
			//catch (Exception e)
			//{
			//    EventLog.WriteEntry("MIDApplicationService", "Error unmarshalling MRSServerPing - " + e.Message, EventLogEntryType.Error);
			//    return;
			//}
			...
			{
				EventLog.WriteEntry("MIDApplicationService", "Error unmarshalling MRSServerObjectFactory - " + e.Message, EventLogEntryType.Error);
				return;
			}
			//End Fix - JScott - Correct Group Dynamite Conneciton problem
			...
			{
				EventLog.WriteEntry("MIDApplicationService", "Error closing port #" + _port + " - " + e.Message, EventLogEntryType.Error);
				return;
			}

			// Service Stopped...

			EventLog.WriteEntry("MIDApplicationService", "MIDApplicationService stopped successfully.", EventLogEntryType.Information);
		}
		...
		private void GarbageCollect()
		{
			...
			{
				EventLog.WriteEntry("MIDApplicationService", "Error encountered -- " + e.Message, EventLogEntryType.Error);
				return;
			}
		}
