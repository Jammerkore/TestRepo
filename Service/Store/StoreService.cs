...
namespace MIDRetail.Service.Store
{
	public class StoreService : System.ServiceProcess.ServiceBase
	{
		/// <summary> 
...
// (CSMITH) - END MID Track #3369
		...
		static void Main()
		{
			...
			//
			ServicesToRun = new System.ServiceProcess.ServiceBase[] { new StoreService() };
		...
		private void InitializeComponent()
		{
			...
			// 
			this.AutoLog = false;
			this.ServiceName = "MIDStoreService";
		...
		protected override void OnStart(string[] args)
		{
			...
			{
				EventLog.WriteEntry("MIDStoreService", "Error configuring Remoting - " + e.Message, EventLogEntryType.Error);
				return;
			}
			...
			//catch (Exception e)
			//{
			//    EventLog.WriteEntry("MIDStoreService", "Error registering StoreServiceSession - " + e.Message, EventLogEntryType.Error);
			//    return;
			//}
...
// (CSMITH) - BEG MID Track #3369: DB Connection String hardcoded in config files
			...
			{
				EventLog.WriteEntry("MIDStoreService", "Database Connection String MUST be specified", EventLogEntryType.Error);
...
//					_channel = new TcpChannel(Convert.ToInt16(_port));
				...
				{
					EventLog.WriteEntry("MIDStoreService", "Error opening port #" + _port + " - " + e.Message, EventLogEntryType.Error);
					return;
				}
			}
			catch (Exception e)
			{
				EventLog.WriteEntry("MIDStoreService", "Error getting port from MyPort on config file - " + e.Message, EventLogEntryType.Error);
				return;
			}
			...
			//catch (Exception e)
			//{
			//    EventLog.WriteEntry("MIDStoreService", "Error creating or marshalling MRSServerPing - " + e.Message, EventLogEntryType.Error);
			//    return;
			//}
			...
			{
				EventLog.WriteEntry("MIDStoreService", "Error creating or marshalling MRSServerObjectFactory - " + e.Message, EventLogEntryType.Error);
				return;
			}
			//End Fix - JScott - Correct Group Dynamite Conneciton problem
			...
			EventLog.WriteEntry("MIDStoreService", "MIDStoreService started successfully and listening on port #" + _port, EventLogEntryType.Information);
		}
		...
		protected override void OnStop()
		{
			...
			if (_gcCollectThread != null)
			{
				...
				{
					EventLog.WriteEntry("MIDStoreService", "Error stopping Garbage Collection thread - " + e.Message, EventLogEntryType.Error);
					return;
				}
			}

            // Begin TT#2307 - JSmith - Incorrect Stock Values
            StoreServerGlobal.MessageProcessor.StopMessageListener();
            // End TT#2307
			...
			{
				EventLog.WriteEntry("MIDStoreService", "Error cleaning up global area - " + e.Message, EventLogEntryType.Error);
				return;
			}
			...
			//catch (Exception e)
			//{
			//    EventLog.WriteEntry("MIDStoreService", "Error unmarshalling MRSServerPing - " + e.Message, EventLogEntryType.Error);
			//    return;
			//}
			...
			{
				EventLog.WriteEntry("MIDStoreService", "Error unmarshalling MRSServerObjectFactory - " + e.Message, EventLogEntryType.Error);
				return;
			}
			//End Fix - JScott - Correct Group Dynamite Conneciton problem
			...
			{
				EventLog.WriteEntry("MIDStoreService", "Error closing port #" + _port + " - " + e.Message, EventLogEntryType.Error);
				return;
			}

			// Service Stopped...

			EventLog.WriteEntry("MIDStoreService", "MIDStoreService stopped successfully.", EventLogEntryType.Information);
		}
		...
		private void GarbageCollect()
		{
			...
			{
				EventLog.WriteEntry("MIDStoreService", "Error encountered -- " + e.Message, EventLogEntryType.Error);
				return;
			}
		}
