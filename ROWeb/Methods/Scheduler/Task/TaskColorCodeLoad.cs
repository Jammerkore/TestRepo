...
{
    public class TaskColorCodeLoad : TaskBase
    {
        //=======
        // FIELDS
        //=======
        ...
        {
            ROTaskColorCodeLoad ROTask = null;
            ...
            if (taskParameters.Sequence < TaskListProperties.Tasks.Count
                && TaskListProperties.Tasks[taskParameters.Sequence] is ROTaskColorCodeLoad)
            {
                ROTask = (ROTaskColorCodeLoad)TaskListProperties.Tasks[taskParameters.Sequence];
            }
            else
            {
                ...
                ROTask = new ROTaskColorCodeLoad(
                    task: task,
        ...
        private void AddValues(ROTaskParms taskParameters, ROTaskColorCodeLoad task)
        {
        ...
        private string TaskDirectory => string.IsNullOrEmpty(MIDConfigurationManager.AppSettings["FileDirectory"]) ? @"C:\Logility\ROData\Color" :
                    string.Concat(MIDConfigurationManager.AppSettings["FileDirectory"], @"\Color");
        ...
        {
            successful = true;
            ROTaskColorCodeLoad taskColorCodeLoadData = (ROTaskColorCodeLoad)taskData;
            ...
            if (!SetTask(taskData: taskColorCodeLoadData, message: ref message))
            {
                successful = false;
            }
