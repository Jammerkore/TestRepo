        ...
        private void AddValues(ROTaskParms taskParameters, ROTaskColorCodeLoad task)
        {
            if (TaskData.Rows.Count > 0)
            {
                ...
                inputDirectory = string.IsNullOrEmpty(inputDirectory) ?
                    string.IsNullOrEmpty(MIDConfigurationManager.AppSettings["FileDirectory"]) ? @"C:\Logility\ROData\Color" :
                    string.Concat(MIDConfigurationManager.AppSettings["FileDirectory"], @"\Color") : inputDirectory;
