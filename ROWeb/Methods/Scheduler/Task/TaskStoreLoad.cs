...
{
    public class TaskStoreLoad : TaskBase
    {
        //=======
        // FIELDS
        //=======
            ...
            )
        {
            ...
            if (taskParameters.Sequence < TaskListProperties.Tasks.Count
                && TaskListProperties.Tasks[taskParameters.Sequence] is ROTaskStoreLoad)
            {
                ROTask = (ROTaskStoreLoad)TaskListProperties.Tasks[taskParameters.Sequence];
            }
            else
            {
        ...
        private void AddValues(ROTaskParms taskParameters, ROTaskStoreLoad task)
        {
        ...
        private string TaskDirectory => string.IsNullOrEmpty(MIDConfigurationManager.AppSettings["FileDirectory"]) ? @"C:\Logility\ROData\Channel" :
                    string.Concat(MIDConfigurationManager.AppSettings["FileDirectory"], @"\Channel");
        ...
        {
            successful = true;
            ROTaskStoreLoad taskStoreLoadData = (ROTaskStoreLoad)taskData;
            ...
            if (!SetTask(taskData: taskStoreLoadData, message: ref message))
            {
                successful = false;
            }
