...
{
    public class TaskHistoryPlanLoad : TaskBase
    {
        //=======
        // FIELDS
        //=======
        ...
        {
            ROTaskHistoryPlanLoad ROTask = null;
            ...
            if (TaskData == null || (TaskData != null && TaskData.Select("TASK_SEQUENCE=" + taskParameters.Sequence).Count() == 0))
            {
                TaskGetValues();
            }
            ...
            if (taskParameters.Sequence < TaskListProperties.Tasks.Count
                && TaskListProperties.Tasks[taskParameters.Sequence] is ROTaskHistoryPlanLoad)
            {
                ROTask = (ROTaskHistoryPlanLoad)TaskListProperties.Tasks[taskParameters.Sequence];
            }
            else
            {
                ...
                ROTask = new ROTaskHistoryPlanLoad(
                    task: task,
        ...
        private void AddValues(ROTaskParms taskParameters, ROTaskHistoryPlanLoad task)
        {
        ...
        private string TaskDirectory => string.IsNullOrEmpty(MIDConfigurationManager.AppSettings["FileDirectory"]) ? @"C:\Logility\ROData\History Forecast" :
                    string.Concat(MIDConfigurationManager.AppSettings["FileDirectory"], @"\History Forecast");
        ...
        {
            successful = true;
            ROTaskHistoryPlanLoad taskHistoryPlanLoadData = (ROTaskHistoryPlanLoad)taskData;
            ...
            if (!SetTask(taskData: taskHistoryPlanLoadData, message: ref message))
            {
                successful = false;
            }
        ...
        private bool SetTask(
            ROTaskHistoryPlanLoad taskData, 
            ref string message
            )
        {
