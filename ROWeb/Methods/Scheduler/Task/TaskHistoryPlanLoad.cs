        ...
        private void AddValues(ROTaskParms taskParameters, ROTaskHistoryPlanLoad task)
        {
            if (TaskData.Rows.Count > 0)
            {
                ...
                inputDirectory = string.IsNullOrEmpty(inputDirectory) ?
                    string.IsNullOrEmpty(MIDConfigurationManager.AppSettings["FileDirectory"]) ? @"C:\Logility\ROData\History Forecast" :
                    string.Concat(MIDConfigurationManager.AppSettings["FileDirectory"], @"\History Forecast") : inputDirectory;
