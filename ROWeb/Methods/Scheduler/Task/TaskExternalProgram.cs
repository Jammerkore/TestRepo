...
{
    public class TaskExternalProgram : TaskBase
    {
        //=======
        // FIELDS
        //=======
        ...
        {
            ROTaskExternalProgram ROTask = null;
            ...
            if (taskParameters.Sequence < TaskListProperties.Tasks.Count
                && TaskListProperties.Tasks[taskParameters.Sequence] is ROTaskExternalProgram)
            {
                ROTask = (ROTaskExternalProgram)TaskListProperties.Tasks[taskParameters.Sequence];
            }
            else
            {
                ...
                ROTask = new ROTaskExternalProgram(
                    task: task,
        ...
        {
            successful = true;
            ROTaskExternalProgram taskExternalProgramData = (ROTaskExternalProgram)taskData;

            if (!SetTask(taskData: taskExternalProgramData, message: ref message))
            {
                successful = false;
            }
        ...
        private bool SetTask(
            ROTaskExternalProgram taskData, 
            ref string message
            )
        {
        ...
        override public bool TaskSaveData(
            ...
            ref string message)
        {
            scheduleDataLayer.TaskProgram_Insert(TaskData);
        ...
        {
            TaskData = ScheduleDataLayer.TaskProgram_ReadByTaskList(aTaskListRID: TaskListKey);
        }
    }
}
