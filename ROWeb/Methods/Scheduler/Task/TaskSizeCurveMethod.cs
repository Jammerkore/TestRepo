...
{
    public class TaskSizeCurveMethod : TaskBase
    {
        //=======
        // FIELDS
        //=======
        ...
        {
            ROTaskSizeCurveMethod ROTask = null;
            ...
            if (taskParameters.Sequence < TaskListProperties.Tasks.Count
                && TaskListProperties.Tasks[taskParameters.Sequence] is ROTaskSizeCurveMethod)
            {
                ROTask = (ROTaskSizeCurveMethod)TaskListProperties.Tasks[taskParameters.Sequence];
            }
            else
            {
                ...
                ROTask = new ROTaskSizeCurveMethod(
                    task: task,
        ...
        private void AddValues(ROTaskParms taskParameters, ROTaskSizeCurveMethod task)
        {
        ...
        {
            successful = true;
            ROTaskSizeCurveMethod taskSizeCurveMethodData = (ROTaskSizeCurveMethod)taskData;

            if (!SetTask(taskData: taskSizeCurveMethodData, message: ref message))
            {
                successful = false;
            }
        ...
        private bool SetTask(
            ROTaskSizeCurveMethod taskData, 
            ref string message
            )
        {
        ...
        override public bool TaskSaveData(
            ...
            ref string message)
        {
            scheduleDataLayer.TaskSizeCurveMethod_Insert(TaskData);
        ...
        {
            TaskData = ScheduleDataLayer.TaskSizeCurveMethod_ReadByTaskList(aTaskListRID: TaskListKey);
        }
    }
}
