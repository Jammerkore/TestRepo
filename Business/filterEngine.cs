...
namespace MIDRetail.Business
{
    public static class filterEngine
    {
        ...
        public static ProfileList RunFilter(filter f, ProfileList aProfileList)
        {
            ...
            ConditionNode cnSortByParent = f.FindConditionNode(3); //parent seq for conditions
            ...
            if (cnSortByParent.ConditionNodes.Count > 0)
            {
                ...
                foreach (ConditionNode cnSort in cnSortByParent.ConditionNodes)
                {
                    ...
                    sortByDirectionTypes sortByDirection = sortByDirectionTypes.FromIndex(cnSort.condition.operatorIndex);
                    if (sortByDirection == sortByDirectionTypes.Ascending)
                    {
                        map1.ascending = true;
                    }
                    else
                    ...
                    sortByTypes sortByType = sortByTypes.FromIndex(cnSort.condition.sortByTypeIndex);
                    int fieldIndex = cnSort.condition.sortByFieldIndex;
                    valueInfoTypes valueInfoType;
                    if (sortByType == sortByTypes.StoreCharacteristics)
                    {
                        valueInfoType = filterDataHelper.StoreCharacteristicsGetValueInfoType(fieldIndex);
                    }
                    else if (sortByType == sortByTypes.HeaderCharacteristics)
                    {
                        valueInfoType = filterDataHelper.HeaderCharacteristicsGetValueInfoType(fieldIndex);
                    }
                    else if (sortByType == sortByTypes.StoreFields)
                    {
                        valueInfoType = storeFieldTypes.GetValueTypeInfoForField(fieldIndex);
                    }
                    else if (sortByType == sortByTypes.HeaderFields)
                    {
                        valueInfoType = headerFieldTypes.GetValueTypeForField(fieldIndex);
                    }
                    else if (sortByType == sortByTypes.StoreStatus)
                    {
                        valueInfoType = new valueInfoTypes(valueTypes.Numeric, numericTypes.Integer);
                    }
                    else if (sortByType == sortByTypes.HeaderStatus)
                    {
                        valueInfoType = new valueInfoTypes(valueTypes.Numeric, numericTypes.Integer);
                    }
                    else if (sortByType == sortByTypes.HeaderType)
                    {
                        valueInfoType = new valueInfoTypes(valueTypes.Numeric, numericTypes.Integer);
                    }
                    else if (sortByType == sortByTypes.HeaderDate)
                    {
                        valueInfoType = new valueInfoTypes(valueTypes.Date, dateTypes.DateOnly);
                    }
                    else if (sortByType == sortByTypes.HeaderReleaseDate)
                    {
                        valueInfoType = new valueInfoTypes(valueTypes.Date, dateTypes.DateAndTime);
                    }
                    else if (sortByType == sortByTypes.Variables)
                    {
                        valueInfoType = filterDataHelper.VariablesGetValueInfoType(fieldIndex);
                    }
                    else
                    {
                        valueInfoType = new valueInfoTypes(valueTypes.Text);
                    }
                    
                    valueTypes valueType = valueInfoType.valueType;
                    numericTypes numericType = valueInfoType.numericType;
                    ...
                    else if (valueType == valueTypes.Numeric)
                    {
            ...
            if (doSorting)
            {
                ...
                foreach (int profKey in tempList)
                {
                    ...
                    foreach (ConditionNode cnSort in cnSortByParent.ConditionNodes)
                    {
                        ...
                        if (sortByType == sortByTypes.StoreCharacteristics)
                        {
                            valueInfoTypes valueInfoType = filterDataHelper.StoreCharacteristicsGetValueInfoType(cnSort.condition.fieldIndex);
                            int scg_rid = cnSort.condition.fieldIndex;
                            ...
                            if (objValue != null)
                            {
                                ...
                                else if (valueType == valueTypes.Dollar)
                                {
                        ...
                        {
                            storeFieldTypes fieldType = storeFieldTypes.FromIndex(cnSort.condition.fieldIndex);
                            StoreEnhancedProfile ep = new StoreEnhancedProfile();
        ...
        //private static System.Windows.Forms.Form frmDisplay = null;
        //private static System.Windows.Forms.ListBox lb;
        ...
        private static bool ExecuteCondition(filter f, filterCondition fc, Profile profileToCompare)
        {
            //if (frmDisplay == null)
            //{
            //    frmDisplay = new System.Windows.Forms.Form();
            //    lb = new System.Windows.Forms.ListBox();
            //    frmDisplay.Controls.Add(lb);
            //    lb.Dock = System.Windows.Forms.DockStyle.Fill;
            //    frmDisplay.Text = "Show Filter Execution Plan";
            //    frmDisplay.Show();
            //}
            
            //filterManager m = new filterManager(f, null, null, null);
            //m.Options.ColorFormatBlackBlueRed = false;
            //m.Options.ColorFormatBlackRed = false;
            //m.Options.ColorFormatGreenBlueRed = false;

            //elementGroup eg = f.GetElementGroupFromCondition(m, fc);
            //eg.BuildFormattedText(m.Options, ref fc);

            //lb.Items.Add(fc.executeGroup + " " + fc.NodeFormattedText);
            ...
            if (f.filterType == filterTypes.HeaderFilter)
            {
                ...
                elementGroupTypes et = elementGroupTypes.FromIndex(fc.elementGroupTypeIndex);
                if (et == elementGroupTypes.HeaderTypes)
                {
                    DataRow[] drHeaderTypes = fc.GetListValues(listValueTypes.HeaderTypes);
            ...
            else if (f.filterType == filterTypes.StoreFilter)
            {
                ...
                StoreEnhancedProfile eProf = new StoreEnhancedProfile();
                eProf.prof = store;

                elementGroupTypes et = elementGroupTypes.FromIndex(fc.elementGroupTypeIndex);
                if (et == elementGroupTypes.StoreFields)
                {          
                    storeFieldTypes fieldType = storeFieldTypes.FromIndex(fc.fieldIndex);                
                    return fieldType.CompareStoreValue(eProf, fc);
                }
                else if (et == elementGroupTypes.StoreCharacteristics)
                {
                    ...
                    valueTypes valueType = valueTypes.FromIndex(fc.valueTypeIndex);
                       // StoreCharGroupProfile scgp = (StoreCharGroupProfile)store.Characteristics[scg_rid];
                ...
                else if (et == elementGroupTypes.StoreAttributeSet)
                {
                    ...
                    if (p != null)                    
                    {
                        listOperatorTypes listOp = listOperatorTypes.FromIndex(fc.operatorIndex);
                        bool b = false;

                        DataRow[] drGroups = fc.GetListValues(listValueTypes.StoreGroupLevel);  //a list of all the groups they selected
                        ...
                        foreach (StoreGroupLevelProfile v in p.GroupLevels) //p.GroupLevels is a list of all current possible groups for this attribute set
                        {
                            ...
                            if (fc.listConstantType == listConstantTypes.All)
                            {
                                searchThisGroup = true;
                            }
                            else if (fc.listConstantType == listConstantTypes.None)
                            {
                            ...
                            if (searchThisGroup)
                            {
                                ...
                                if (a != null)
                                {
                                    //we found a group that has contains our store
                                    b = true;
                                    break;
                                }
                        ...
                        if (listOp == listOperatorTypes.Excludes)
                        {
                            b = !b;
                        }
                        return b;
                ...
                else if (et == elementGroupTypes.StoreList)
                {                   
                    int storeRID = eProf.prof.Key;
                    return filterDataHelper.CompareInList(listValueTypes.StoreRID, storeRID, fc);
                }
                else if (et == elementGroupTypes.StoreStatus)
                {
                    int storeStatus = (int)eProf.prof.Status;
                    return filterDataHelper.CompareInList(listValueTypes.StoreStatus, storeStatus, fc);
                }
                else if (et == elementGroupTypes.StoreVariableToConstant)
                {
                    ...
                    {
                        bool b = ProcessVariableToConstant(localPlanCubeGroup, f, fc, varDateRangeProfile, nodeProfile, versionProfile, store, currentPlanOpenParms, dateProf);
                        if (b == true)
                        {
                            return b;
                        }
                ...
                else if (et == elementGroupTypes.StoreVariableToVariable)
                {
                    PlanOpenParms currentPlanOpenParms = null;
                    if (f.currentPlanCubeGroup != null)
                    {
                ...
                else if (et == elementGroupTypes.StoreVariablePercentage)
                {
                    PlanOpenParms currentPlanOpenParms = null;
                    if (f.currentPlanCubeGroup != null)
                    {
                    ...
                    foreach (DateProfile dateProf in dateProfileList)
                    {
                        bool b = ProcessVariablePercentage(f, fc, store, currentPlanOpenParms, dateProf, localPlanCubeGroup, varDateRangeProfile, nodeProfile, versionProfile, var2_localPlanCubeGroup, var2_DateRangeProfile, var2_nodeProfile, var2_versionProfile);
                        if (b == true)
                        {
                            return b;
                        }
        ...
        private static bool ProcessVariableToConstant(PlanCubeGroup localPlanCubeGroup, filter f, filterCondition fc, DateRangeProfile varDateRangeProfile, HierarchyNodeProfile nodeProfile, VersionProfile versionProfile, StoreProfile storeProfile, PlanOpenParms currentPlanOpenParms, DateProfile currentDateProfile)
        {
            ...
            if (fc.valueToCompare != null) //special handling of Grade variable
            {
            ...
            return EvaluateCondition(firstValues, secondValues, variableTimeTypes.FromIndex(fc.variable1_TimeTypeIndex), variableTimeTypes.Any, fc);
        }

        private static bool ProcessVariableToVariable(filter f, filterCondition fc, StoreProfile storeProfile, PlanOpenParms currentPlanOpenParms, DateProfile currentDateProfile, PlanCubeGroup var1_localPlanCubeGroup, DateRangeProfile var1_DateRangeProfile, HierarchyNodeProfile var1_nodeProfile, VersionProfile var1_versionProfile, PlanCubeGroup var2_localPlanCubeGroup, DateRangeProfile var2_DateRangeProfile, HierarchyNodeProfile var2_nodeProfile, VersionProfile var2_versionProfile)
        {
            double[] firstValues = GetVariableValuesFromCondition(var1_localPlanCubeGroup, true, f, fc, var1_DateRangeProfile, var1_nodeProfile, var1_versionProfile, storeProfile, currentPlanOpenParms, currentDateProfile);

         
            double[] secondValues = GetVariableValuesFromCondition(var2_localPlanCubeGroup, false, f, fc, var2_DateRangeProfile, var2_nodeProfile, var2_versionProfile, storeProfile, currentPlanOpenParms, currentDateProfile);

            return EvaluateCondition(firstValues, secondValues, variableTimeTypes.FromIndex(fc.variable1_TimeTypeIndex), variableTimeTypes.FromIndex(fc.variable2_TimeTypeIndex), fc);
        }
        ...
        private static bool ProcessVariablePercentage(filter f, filterCondition fc, StoreProfile storeProfile, PlanOpenParms currentPlanOpenParms, DateProfile currentDateProfile, PlanCubeGroup var1_localPlanCubeGroup, DateRangeProfile var1_DateRangeProfile, HierarchyNodeProfile var1_nodeProfile, VersionProfile var1_versionProfile, PlanCubeGroup var2_localPlanCubeGroup, DateRangeProfile var2_DateRangeProfile, HierarchyNodeProfile var2_nodeProfile, VersionProfile var2_versionProfile)
        {
            ...
            double[] pctValues = GetVariableValuesFromCondition(var2_localPlanCubeGroup, false, f, fc, var2_DateRangeProfile, var2_nodeProfile, var2_versionProfile, storeProfile, currentPlanOpenParms, currentDateProfile);

            
            double[] secondValues = new double[0];
            secondValues[0] = (double)fc.valueToCompareDouble;

            percentageOperatorTypes percentOp = percentageOperatorTypes.FromIndex(fc.operatorVariablePercentageIndex);

            bool doPercentOf = false;
            bool doPercentChange = false;
            if (percentOp == percentageOperatorTypes.PercentOf)
            {
                doPercentOf = true;
            }
            if (percentOp == percentageOperatorTypes.PercentChange)
            {
                doPercentChange = true;
            }

            return EvaluatePctCondition(firstValues, pctValues, secondValues, variableTimeTypes.FromIndex(fc.variable1_TimeTypeIndex), variableTimeTypes.FromIndex(fc.variable2_TimeTypeIndex), variableTimeTypes.Any, fc, doPercentOf, doPercentChange);
        }
            ...
            double val2)
        {
            ...
            try
            {
                ...
                numericOperatorTypes numericOp = numericOperatorTypes.FromIndex(fc.operatorIndex);

                if (numericOp == numericOperatorTypes.DoesEqual)
                {
                    return (val1 == val2);
                }
                else if (numericOp == numericOperatorTypes.DoesNotEqual)
                {
                    return (val1 != val2);
                }
                else if (numericOp == numericOperatorTypes.GreaterThan)
                {
                    return (val1 > val2);
                }
                else if (numericOp == numericOperatorTypes.GreaterThanOrEqualTo)
                {
                    return (val1 >= val2);
                }
                else if (numericOp == numericOperatorTypes.LessThan)
                {
                    return (val1 < val2);
                }
                else if (numericOp == numericOperatorTypes.LessThanOrEqualTo)
                {
                    return (val1 <= val2);
                }
                else if (numericOp == numericOperatorTypes.Between)
                {
        ...
        private static double[] GetVariableValuesFromCondition(PlanCubeGroup localPlanCubeGroup, bool useVar1, filter f, filterCondition fc, DateRangeProfile varDateRangeProfile, HierarchyNodeProfile nodeProfile, VersionProfile versionProfile, StoreProfile storeProfile, PlanOpenParms currentPlanOpenParms, DateProfile currentDateProfile)
        {
            ...
            return GetValuesForVariable(isTimeTotalVariable, varIndex, valueType, timeType, localPlanCubeGroup, nodeProfile, versionProfile, varDateRangeProfile, storeProfile, detailDateProfList);

        }

        private static double[] GetValuesForVariable(bool isTimeTotalVariable, int variableIndex, variableValueTypes variableValueType, variableTimeTypes variableTimeType, PlanCubeGroup localPlanCubeGroup, HierarchyNodeProfile nodeProfile, VersionProfile versionProfile, DateRangeProfile varDateRangeProfile, StoreProfile aStoreProf, ProfileList aDetailDateProfList)
        {
        ...
        private static bool CompareValues(filterCondition fc, double val1, double val2)
		{
			try
			{
                numericOperatorTypes numericOp = numericOperatorTypes.FromIndex(fc.operatorIndex);
        ...
        private static ProfileList SetDateRangeProfileForVariable(SessionAddressBlock SAB, DateRangeProfile varDateRangeProfile, variableTimeTypes variableTimeType, StoreProfile aStoreProf, DateProfile aCurrentDateProf)
        {
