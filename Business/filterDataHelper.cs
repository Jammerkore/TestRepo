    ...
    public static class filterDataHelper
    {
        ...
        public static filter LoadExistingFilter(int filterRID)
        {
            ...
            foreach (DataRow drCondition in dtConditions.Rows)
            {
                ...
                if (cn.condition.listConstantType == listConstantTypes.HasValues)
                {
        ...
        public static bool CompareInList(listValueTypes listValueType, int valueIndexToCompare, filterCondition fc)
        {
        ...
        public static bool CompareToString(string val1, filterCondition fc)
        {
            ...
            stringOperatorTypes stringOp = stringOperatorTypes.FromIndex(fc.operatorIndex);

       
            
            if (stringOp == stringOperatorTypes.Contains)
            {
                return val1.ToUpper().Contains(val2.ToUpper());
            }
            else if (stringOp == stringOperatorTypes.ContainsExactly)
            {
                return val1.Contains(val2);
            }
            else if (stringOp == stringOperatorTypes.StartsWith)
            {
                return val1.ToUpper().StartsWith(val2.ToUpper());
            }
            else if (stringOp == stringOperatorTypes.StartsExactlyWith)
            {
                return val1.StartsWith(val2);
            }
            else if (stringOp == stringOperatorTypes.EndsWith)
            {
                return val1.ToUpper().EndsWith(val2.ToUpper());
            }
            else if (stringOp == stringOperatorTypes.EndsExactlyWith)
            {
                return val1.EndsWith(val2);
            }
            else if (stringOp == stringOperatorTypes.Equals)
            {
                return (val1.ToUpper() == val2.ToUpper());
            }
            else if (stringOp == stringOperatorTypes.EqualsExactly)
            {
                return (val1 == val2);
            }
            else
        ...
        {
            bool val2 = fc.valueToCompareBool;
            return (val1 == val2);
        }
        ...
        public static bool CompareToDate(DateTime val1, filterCondition fc)
        {
            ...
            dateOperatorTypes dateOp = dateOperatorTypes.FromIndex(fc.operatorIndex);
            ...
            if (dateOp == dateOperatorTypes.Unrestricted)
            {
                return true;
            }
            else
            {
                int i1;
                if (fc.dateTypeIndex == dateTypes.DateOnly)
                {
                    i1 = DateTime.Compare(val1.Date, dtCompareFrom.Date);
                }
                else if (fc.dateTypeIndex == dateTypes.TimeOnly)
                {
        ...
        public static DataTable AttributeSetGetValuesForGroup(int key)
        {
            ...
            //ProfileList a1 = SAB.ApplicationServerSession.GetProfileList(eProfileType.StoreGroupListView, !userAttrSecLvl.AccessDenied);
            StoreGroupProfile p = SAB.StoreServerSession.GetStoreGroup(key);
        ...
        public static valueInfoTypes AttributeSetGetValueInfoType(int key)
        {
            valueInfoTypes v = new valueInfoTypes(valueTypes.List);
            return v;
        }
        ...
        public static valueInfoTypes VariablesGetValueInfoType(int fieldIndex)
        {
            ...
            {
                valueTypes vt = valueTypes.Numeric;
                numericTypes nt = numericTypes.DoubleFreeForm;
                vInfo = new valueInfoTypes(vt, nt);
            }
