...
{
	/// <summary>
	/// This class defines the PlanPlanPeriodDetailCube.
	/// </summary>
	/// <remarks>
	/// The PlanPlanPeriodDetailCube defines a cube that contains the plan Period detail values.
	/// </remarks>

	abstract public class PlanPlanPeriodDetailCube : PlanPlanCube
	{
		//=======
		// FIELDS
		//=======
		...
		/// <summary>
		/// Creates a new instance of PlanPlanPeriodDetailCube, using the given SessionAddressBlock, Transaction, and PlanCubeGroup.
		/// </summary>
		/// <param name="aSAB">
		/// A reference to a SessionAddressBlock that this PlanPlanPeriodDetailCube is a part of.
		/// </param>
		/// <param name="aTransaction">
		/// A reference to a Transaction that this PlanPlanPeriodDetailCube is a part of.
		/// </param>
		/// <param name="aPlanCubeGroup">
		/// A reference to a PlanCubeGroup that this PlanPlanPeriodDetailCube is a part of.
		/// </param>

		public PlanPlanPeriodDetailCube(
			...
			: base(aSAB, aTransaction, aPlanCubeGroup, aCubeDef, (ushort)(aCubeAttributes | PlanCubeAttributesFlagValues.PeriodDetail), aCubePriority, aReadOnly, aCheckNodeSecurity)
		{
		...
		override public bool canCellBeScheduled(ComputationCellReference aCompCellRef)
		{
			...
			try
			{
				...
				firstTimelineKey = dateList[0].Key;
				lastTimelineKey = dateList[dateList.Count - 1].Key;

				periodProf = planCellRef.PlanCube.SAB.ApplicationServerSession.Calendar.GetPeriod(planCellRef[GetTimeType()]);
				firstWeekKey = periodProf.Weeks[0].Key;
				lastWeekKey = periodProf.Weeks[periodProf.Weeks.Count - 1].Key;

				if ((firstWeekKey >= firstTimelineKey && firstWeekKey <= lastTimelineKey) ||
					(lastWeekKey >= firstTimelineKey && lastWeekKey <= lastTimelineKey) ||
					(firstWeekKey < firstTimelineKey && lastWeekKey > lastTimelineKey))
				//End Track #6007 - JScott - All Cum Variables when Sales are changed at Month or Quarter are not cascading.
				{
					return true;
				}
		...
		override public bool isVersionProtected(PlanCellReference aPlanCellRef)
		{
			...
			try
			{
				hashKey = new HashKeyObject(aPlanCellRef[eProfileType.Version], aPlanCellRef[eProfileType.HierarchyNode], aPlanCellRef[eProfileType.Period], aPlanCellRef[eProfileType.Variable]);
