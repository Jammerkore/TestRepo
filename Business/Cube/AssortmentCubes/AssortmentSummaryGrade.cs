...
{
	public class AssortmentSummaryGrade : AssortmentSummaryCube
	{
		//=======
		// FIELDS
		//=======
		...
		public AssortmentSummaryGrade(SessionAddressBlock aSAB, ApplicationSessionTransaction aTransaction, AssortmentCubeGroup aAssrtCubeGroup, CubeDefinition aCubeDefinition, int aCubePriority, bool aReadOnly, bool aCheckNodeSecurity)
				...
				if (aSummaryTable.Rows.Count > 0)
				{
					...
					foreach (System.Data.DataRow dataRow in aSummaryTable.Rows)
					{
						...
						if (strGrpLvlRID != -1 && strGrdRID != -1)
						{
							...
							assrtCellRef[eProfileType.StoreGrade] = Convert.ToInt32(dataRow["GRADE_RID"], CultureInfo.CurrentUICulture);
							...
							foreach (AssortmentSummaryVariableProfile varProf in varProfList)
							{
								if (varProf.DatabaseColumnName != null)
								{
                                    ...
                                    //Debug.WriteLine("AssortmentSummaryGrade.LoadCube()  Cell Val: " + assrtCellRef.CurrentCellValue +
                                    //    " DB Val: " + Convert.ToDouble(dataRow[varProf.DatabaseColumnName]) +
                                    //    "  " + ((AssortmentCellReference)assrtCellRef).CellKeys +
                                    //    "  isCellLoadedFromDB: " + assrtCellRef.isCellLoadedFromDB + " reloadFromDB: " + reloadFromDB);
		...
		override public string GetCellDescription(ComputationCellReference aCompCellRef)
		{
			...
			try
			{
				...
				return "Assortment Summary Grade" +
