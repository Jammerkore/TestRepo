    ...
    public static class filterEngineSQLforAudit
    {
        ...
        private static void MakeProductHierarchyForCondition(ConditionNode cn, ref string sSQL)
        {
            ...
            if (et == filterDictionary.ProductMerchandise)
            {
                ...
                sSQL += htab1 + "SELECT @PH_RID_" + GetConditionUID(fc) + ", HN_RID FROM UDF_HIERARCHY_GET_FROM_NODE(" + cn.condition.headerMerchandise_HN_RID.ToString() + ", @PH_RID_" + GetConditionUID(fc) + ")" + System.Environment.NewLine;
