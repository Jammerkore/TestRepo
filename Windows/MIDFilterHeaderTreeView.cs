...
{
    public class FilterHeaderTreeView : MIDTreeView
    {
        //=======
        // FIELDS
        //=======
        ...
        private filterTypes treeFilterType = filterTypes.HeaderFilter;

        //=============
        // CONSTRUCTORS
        //=============

        public FilterHeaderTreeView()
        {
        ...
        override public void InitializeTreeView(SessionAddressBlock aSAB, bool aAllowMultiSelect, Form aMDIParentForm)
        {
            try
            {
                ...
                _secGrpHash[SAB.ClientServerSession.UserRID] = new MIDTreeNodeSecurityGroup(
                    SAB.ClientServerSession.GetMyUserFunctionSecurityAssignment(eSecurityFunctions.ToolsFiltersHeaderUser),
                    SAB.ClientServerSession.GetMyUserFunctionSecurityAssignment(eSecurityFunctions.ExplorersFilterHeaderFoldersUser));

                _secGrpHash[Include.GlobalUserRID] = new MIDTreeNodeSecurityGroup(
                    SAB.ClientServerSession.GetMyUserFunctionSecurityAssignment(eSecurityFunctions.ToolsFiltersHeaderGlobal),
                    SAB.ClientServerSession.GetMyUserFunctionSecurityAssignment(eSecurityFunctions.ExplorersFilterHeaderFoldersGlobal));
        ...
        override public void LoadNodes()
        {
            ...
            try
            {
                ...
                //{
                // End TT#107
                dtFolders = _dlFolder.Folder_Read(SAB.ClientServerSession.UserRID, eProfileType.FilterHeaderMainFavoritesFolder);
                ...
                {
                    folderProf = new FolderProfile(Include.NoRID, SAB.ClientServerSession.UserRID, eProfileType.FilterHeaderMainFavoritesFolder, "My Favorites", SAB.ClientServerSession.UserRID);
                ...
                if (!UserSecLvl.AccessDenied)
                {
                    dtFolders = _dlFolder.Folder_Read(SAB.ClientServerSession.UserRID, eProfileType.FilterHeaderMainUserFolder);
                    ...
                    //End TT#677 - JScott - Tasklists disappearing from Tasklist Explorer
                    {
                        folderProf = new FolderProfile(Include.NoRID, SAB.ClientServerSession.UserRID, eProfileType.FilterHeaderMainUserFolder, "My Filters", SAB.ClientServerSession.UserRID);
                ...
                {
                    dtFolders = _dlFolder.Folder_Read(Include.GlobalUserRID, eProfileType.FilterHeaderMainGlobalFolder);

                    if (dtFolders == null)
                    {
                        throw new Exception("Global Header Filter Folder not defined");
                    }
                    else if (dtFolders.Rows.Count != 1)
                    {
                        throw new Exception("More than one Global Header Filter Folder is defined");
                    }
                ...
                folderTypeList.Add((int)eProfileType.FilterHeaderMainFavoritesFolder);
                folderTypeList.Add((int)eProfileType.FilterHeaderMainUserFolder);
                folderTypeList.Add((int)eProfileType.FilterHeaderMainGlobalFolder);
                folderTypeList.Add((int)eProfileType.FilterHeaderSubFolder);

                if (userRIDList.Count > 0)
                {
                    dtFilters = _dlFilters.FilterReadParent(treeFilterType, eProfileType.FilterHeader, userRIDList);
                    dtFolders = _dlFolder.Folder_Read(userRIDList, eProfileType.FilterHeaderSubFolder, true, false);
                    dtFolderItems = _dlFolder.Folder_Item_Read(userRIDList, folderTypeList, eProfileType.FilterHeader, true, false);
                    dtFilterShortcuts = _dlFolder.Folder_Shortcut_Item_Read(userRIDList, eProfileType.FilterHeader);
                ...
                dtUserFolders = _dlFolder.Folder_Read(userRIDList, eProfileType.FilterHeaderMainUserFolder, false, true);
                ...
                if (dtUserFolders.Rows.Count > 0)
                {
                    ...
                    dtFilters = _dlFilters.FilterReadParent(treeFilterType, eProfileType.FilterHeader, userRIDList);
                    dtFolders = _dlFolder.Folder_Read(userRIDList, eProfileType.FilterHeaderSubFolder, false, true);
                    dtFolderItems = _dlFolder.Folder_Item_Read(userRIDList, folderTypeList, eProfileType.FilterHeader, false, true);
        ...
        override protected MIDTreeNode CreateNewFolder(MIDTreeNode aNode, int aUserId)
        {
            ...
            try
            {
                newNodeName = FindNewFolderName("New Folder", aUserId, aNode.Profile.Key, eProfileType.FilterHeaderSubFolder);

                _dlFolder.OpenUpdateConnection();

                try
                {
                    newFolderProf = new FolderProfile(Include.NoRID, aUserId, eProfileType.FilterHeaderSubFolder, newNodeName, aUserId);
                    newFolderProf.Key = _dlFolder.Folder_Create(newFolderProf.UserRID, newFolderProf.Name, newFolderProf.FolderType);
                    _dlFolder.Folder_Item_Insert(aNode.Profile.Key, newFolderProf.Key, eProfileType.FilterHeaderSubFolder);
        ...
        {
            int key;
            HeaderFilterProfile filterProf;
            FolderProfile folderProf;
            ...
            try
            {
                ...
                {
                    case eProfileType.FilterHeader:

                        objEnqueue = EnqueueObject((HeaderFilterProfile)aNode.Profile, false);
                        ...
                        try
                        {
                            ...
                            filterProf = (HeaderFilterProfile)aNode.Profile;
                    ...
                    case eProfileType.FilterHeaderMainFavoritesFolder:
                    case eProfileType.FilterHeaderMainUserFolder:
                    case eProfileType.FilterHeaderMainGlobalFolder:
                    case eProfileType.FilterHeaderSubFolder:
        ...
        private bool _executeAfterEditing = false; //TT#1313-MD -jsobek -Header Filters  
        /// <summary>
        ...
        override protected void EditNode(MIDTreeNode aNode)
        {
            ...
            try
            {
                if (aNode != null)
                {
                    if (aNode.Profile.ProfileType == eProfileType.FilterHeader)
                    {
                        node = (MIDFilterNode)aNode;

                        filterProf = (HeaderFilterProfile)node.Profile;
                        ...
                        if (objEnqueue != null)
                        {
                            ...
                            filterProperties = SharedRoutines.GetFilterFormForExistingFilter(filterProf.Key, SAB, EAB, isReadOnly, _executeAfterEditing); //TT#1313-MD -jsobek -Header Filters  
        ...
        {
            return aClipboardDataType == eProfileType.FilterHeader;
        }
                ...
                if (aChangedNode != null)
                {
                    ...
                    foreach (MIDFilterNode node in aStartNode.Nodes)
                    {
                        if (node.Profile.Key == aChangedNode.Profile.Key && node.Profile.ProfileType == aChangedNode.Profile.ProfileType)
                        {
                            ...
                            if (node.Profile.ProfileType != eProfileType.FilterHeader)
                            {
                        ...
                        else if (node.NodeProfileType == eProfileType.FilterHeaderSubFolder || node.isFolderShortcut)
                        {
        ...
        override public bool isDropAllowed(DragDropEffects aDropAction, MIDTreeNode aDropNode, ClipboardListBase aSelectedNodes)
        {
            try
            {
                if (aSelectedNodes.ClipboardDataType == eProfileType.FilterHeader ||
                    aSelectedNodes.ClipboardDataType == eProfileType.FilterHeaderSubFolder)
                {
        ...
        public void DeleteShortcuts(MIDTreeNode aStartNode, MIDTreeNode aDeleteNode)
        {
            ...
            try
            {
                ...
                foreach (MIDFilterNode node in deleteList)
                {
                    ...
                    else if (node.NodeProfileType == eProfileType.FilterHeaderSubFolder ||
                        node.isFolderShortcut ||
                        (node.isChildShortcut && node.Profile.ProfileType == eProfileType.FilterHeaderSubFolder))
                    {
                ...
                foreach (DataRow row in folderItemList)
                {
                    switch ((eProfileType)Convert.ToInt32(row["CHILD_ITEM_TYPE"]))
                    {
                        ...
                        case eProfileType.FilterHeader:
                            ...
                            // End TT#437-MD - JSmith - Client application will not start due to a missing item on an explorer (Null reference on Client App startup.)
                            filterProf = new HeaderFilterProfile(itemRow);
        ...
        public MIDFilterNode BuildFilterNode(HeaderFilterProfile prof, MIDTreeNode aParentNode, int aUserId)
        {
            MIDFilterNode newNode;
        ...
        private MIDFilterNode MoveFilterNode(MIDFilterNode aFromNode, MIDFilterNode aToNode)
        {
            ...
            try
            {
                ...
                else if (aFromNode.NodeProfileType == eProfileType.FilterHeaderSubFolder)
                {
                    ...
                    if (aToNode.GetTopSourceNode() != aFromNode.GetTopSourceNode())
                    {
                        ...
                        folderProf.Name = FindNewFolderName(folderProf.Name, aToNode.UserId, aToNode.Profile.Key, eProfileType.FilterHeaderSubFolder);
                ...
                else if (aFromNode.NodeProfileType == eProfileType.FilterHeader)
                {
                    filterProf = (HeaderFilterProfile)aFromNode.Profile;
        ...
        private MIDFilterNode JustMoveFilterNodeAndDoNotRename(MIDFilterNode aFromNode, MIDFilterNode aToNode)
        {
            ...
            try
            {
                ...
                if (aFromNode.NodeProfileType == eProfileType.FilterHeader)
                {
                    filterProf = (HeaderFilterProfile)aFromNode.Profile;
        ...
        private MIDFilterNode CopyFilterNode(MIDFilterNode aFromNode, MIDFilterNode aToNode, bool aFindUniqueName)
        {
            ...
            try
            {
                if (aFromNode.isSubFolder)
                {
                    ...
                    try
                    {
                        folderProf.Key = _dlFolder.Folder_Create(folderProf.UserRID, folderProf.Name, eProfileType.FilterHeaderSubFolder);
                        _dlFolder.Folder_Item_Insert(aToNode.Profile.Key, folderProf.Key, eProfileType.FilterHeaderSubFolder);
                ...
                {
                    filterProf = (HeaderFilterProfile)((HeaderFilterProfile)aFromNode.Profile).Clone();
                    filterProf.UserRID = aToNode.UserId;
                    filterProf.OwnerUserRID = aToNode.UserId;
                    ...
                    try
                    {
                        ...
                        this.EAB.AllocationWorkspaceExplorer.BindFilterComboBox();
            ...
            try
            {
                if (aNode.isObject)
                {
                    ...
                    try
                    {
                        ...
                        //try
                        //{
                        //    _dlFolder.Folder_Item_Delete(aNode.Profile.Key, eProfileType.FilterHeader);
                        //    _dlFolder.Folder_Shortcut_DeleteAll(aNode.Profile.Key, eProfileType.FilterHeader);
                        //    _dlFilters.FilterDelete(aNode.Profile.Key);
                        //    _dlFilters.RemoveFilterProcedure(aNode.Profile.Key, filterTypes.HeaderFilter);
                        ...
                        EAB.AllocationWorkspaceExplorer.BindFilterComboBox();
                ...
                else if (aNode.isSubFolder)
                {
                    ...
                    if (aNode.Nodes.Count == 0)
                    {
                        ...
                        try
                        {
                            _dlFolder.Folder_Item_Delete(aNode.Profile.Key, eProfileType.FilterHeaderSubFolder);
                            _dlFolder.Folder_Delete(aNode.Profile.Key, eProfileType.FilterHeaderSubFolder);
                            _dlFolder.CommitData();
                        }
                        catch (Exception exc)
                        {
                ...
                else if (aNode.isObjectShortcut)
                {
                    ...
                    try
                    {
                        _dlFolder.Folder_Shortcut_Delete(aNode.ParentId, aNode.Profile.Key, eProfileType.FilterHeader);
                        _dlFolder.CommitData();
                    }
                    catch (DatabaseForeignKeyViolation)
                    {
                ...
                else if (aNode.isFolderShortcut)
                {
                    ...
                    try
                    {
                        _dlFolder.Folder_Shortcut_Delete(aNode.ParentId, aNode.Profile.Key, eProfileType.FilterHeaderSubFolder);
                        _dlFolder.CommitData();
                    }
                    catch (DatabaseForeignKeyViolation)
                    {
        ...
        // End TT#5437 - JSmith - Copying Global Header Folder w filters attached to My filters get Null error  
        {
            ...
            try
            {
                ...
                if (node == null)
                {
                    ...
                    try
                    {
                        secAdmin.DeleteUserItemByTypeAndRID((int)eProfileType.FilterHeader, f.filterRID);
                        secAdmin.AddUserItem(userRID, (int)eProfileType.FilterHeader, f.filterRID, ownerRID);
                        secAdmin.CommitData();
                    }
                    catch (Exception exc)
                    {
                    ...
                    try
                    {
                        _dlFolder.Folder_Item_Insert(parentNode.NodeRID, f.filterRID, eProfileType.FilterHeader);
                        _dlFolder.CommitData();
                    }
                    catch (Exception exc)
                    {
                    ...
                    HeaderFilterProfile prof = new HeaderFilterProfile(f.filterRID);
                ...
                else
                {
                    ...
                    HeaderFilterProfile prof = (HeaderFilterProfile)node.Profile;
                    prof.Name = f.filterName;
                    prof.UserRID = f.userRID; //TT#4392 -jsobek Changing Header Filter from User to Global causes Unhandled Exception
                    prof.OwnerUserRID = f.ownerUserRID; //TT#4392 -jsobek Changing Header Filter from User to Global causes Unhandled Exception
                    prof.IsLimited = f.isLimited;
                    prof.ResultLimit = (int)f.resultLimit;
                    ...
                    if (f.ownerUserRID == Include.GlobalUserRID)
                    {
                        ...
                        if (parentNode != _globalNode && parentNode.isChildOf(_globalNode) == false)
                        {
                            ...
                            if (SelectedNode != null) //TT#4392 -jsobek Changing Header Filter from User to Global causes Unhandled Exception
                            {
