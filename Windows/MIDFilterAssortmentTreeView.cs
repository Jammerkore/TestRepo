...
{
    public class FilterAssortmentTreeView : MIDTreeView
    {
        //=======
        // FIELDS
        //=======
        ...
        public FilterAssortmentTreeView()
        {
        ...
        override public void InitializeTreeView(SessionAddressBlock aSAB, bool aAllowMultiSelect, Form aMDIParentForm)
        {
            try
            {
                ...
                _secGrpHash[SAB.ClientServerSession.UserRID] = new MIDTreeNodeSecurityGroup(
                    SAB.ClientServerSession.GetMyUserFunctionSecurityAssignment(eSecurityFunctions.ToolsFiltersAssortmentUser),
                    SAB.ClientServerSession.GetMyUserFunctionSecurityAssignment(eSecurityFunctions.ExplorersFilterAssortmentFoldersUser));

                _secGrpHash[Include.GlobalUserRID] = new MIDTreeNodeSecurityGroup(
                    SAB.ClientServerSession.GetMyUserFunctionSecurityAssignment(eSecurityFunctions.ToolsFiltersAssortmentGlobal),
                    SAB.ClientServerSession.GetMyUserFunctionSecurityAssignment(eSecurityFunctions.ExplorersFilterAssortmentFoldersGlobal));
        ...
        /// <summary>
        /// Method that loads the nodes for the MIDTreeView
        /// </summary>
        override public void LoadNodes()
        {
            ...
            try
            {
                ...
                //{
                // End TT#107
                dtFolders = _dlFolder.Folder_Read(SAB.ClientServerSession.UserRID, eProfileType.FilterAssortmentMainFavoritesFolder);
                ...
                {
                    folderProf = new FolderProfile(Include.NoRID, SAB.ClientServerSession.UserRID, eProfileType.FilterAssortmentMainFavoritesFolder, "My Favorites", SAB.ClientServerSession.UserRID);
                ...
                if (!UserSecLvl.AccessDenied)
                {
                    dtFolders = _dlFolder.Folder_Read(SAB.ClientServerSession.UserRID, eProfileType.FilterAssortmentMainUserFolder);
                    ...
                    //End TT#677 - JScott - Tasklists disappearing from Tasklist Explorer
                    {
                        folderProf = new FolderProfile(Include.NoRID, SAB.ClientServerSession.UserRID, eProfileType.FilterAssortmentMainUserFolder, "My Filters", SAB.ClientServerSession.UserRID);
                ...
                {
                    dtFolders = _dlFolder.Folder_Read(Include.GlobalUserRID, eProfileType.FilterAssortmentMainGlobalFolder);

                    if (dtFolders == null)
                    {
                        throw new Exception("Global Assortment Filter Folder not defined");
                    }
                    else if (dtFolders.Rows.Count != 1)
                    {
                        throw new Exception("More than one Global Assortment Filter Folder is defined");
                    }
                ...
                folderTypeList.Add((int)eProfileType.FilterAssortmentMainFavoritesFolder);
                folderTypeList.Add((int)eProfileType.FilterAssortmentMainUserFolder);
                folderTypeList.Add((int)eProfileType.FilterAssortmentMainGlobalFolder);
                folderTypeList.Add((int)eProfileType.FilterAssortmentSubFolder);

                if (userRIDList.Count > 0)
                {
                    dtFilters = _dlFilters.FilterReadParent(treeFilterType, eProfileType.FilterAssortment, userRIDList);
                    dtFolders = _dlFolder.Folder_Read(userRIDList, eProfileType.FilterAssortmentSubFolder, true, false);
                    dtFolderItems = _dlFolder.Folder_Item_Read(userRIDList, folderTypeList, eProfileType.FilterAssortment, true, false);
                    dtFilterShortcuts = _dlFolder.Folder_Shortcut_Item_Read(userRIDList, eProfileType.FilterAssortment);
                ...
                dtUserFolders = _dlFolder.Folder_Read(userRIDList, eProfileType.FilterAssortmentMainUserFolder, false, true);
                ...
                if (dtUserFolders.Rows.Count > 0)
                {
                    ...
                    dtFilters = _dlFilters.FilterReadParent(treeFilterType, eProfileType.FilterAssortment, userRIDList);
                    dtFolders = _dlFolder.Folder_Read(userRIDList, eProfileType.FilterAssortmentSubFolder, false, true);
                    dtFolderItems = _dlFolder.Folder_Item_Read(userRIDList, folderTypeList, eProfileType.FilterAssortment, false, true);
        ...
        override protected MIDTreeNode CreateNewFolder(MIDTreeNode aNode, int aUserId)
        {
            ...
            try
            {
                newNodeName = FindNewFolderName("New Folder", aUserId, aNode.Profile.Key, eProfileType.FilterAssortmentSubFolder);

                _dlFolder.OpenUpdateConnection();

                try
                {
                    newFolderProf = new FolderProfile(Include.NoRID, aUserId, eProfileType.FilterAssortmentSubFolder, newNodeName, aUserId);
                    newFolderProf.Key = _dlFolder.Folder_Create(newFolderProf.UserRID, newFolderProf.Name, newFolderProf.FolderType);
                    _dlFolder.Folder_Item_Insert(aNode.Profile.Key, newFolderProf.Key, eProfileType.FilterAssortmentSubFolder);
        ...
        {
            int key;
            AssortmentFilterProfile filterProf;
            FolderProfile folderProf;
            ...
            try
            {
                ...
                {
                    case eProfileType.FilterAssortment:

                        objEnqueue = EnqueueObject((AssortmentFilterProfile)aNode.Profile, false);
                        ...
                        try
                        {
                            ...
                            filterProf = (AssortmentFilterProfile)aNode.Profile;
                    ...
                    case eProfileType.FilterAssortmentMainFavoritesFolder:
                    case eProfileType.FilterAssortmentMainUserFolder:
                    case eProfileType.FilterAssortmentMainGlobalFolder:
                    case eProfileType.FilterAssortmentSubFolder:
        ...
        override protected void EditNode(MIDTreeNode aNode)
        {
            ...
            try
            {
                if (aNode != null)
                {
                    if (aNode.Profile.ProfileType == eProfileType.FilterAssortment)
                    {
                        node = (MIDFilterNode)aNode;

                        filterProf = (AssortmentFilterProfile)node.Profile;
                        objEnqueue = EnqueueObject(filterProf, true);

                        if (objEnqueue != null)
                        {
                            //filterProperties = new frmFilterProperties(SAB, (MIDFilterNode)node.Parent, _userNode, _globalNode, (AssortmentFilterProfile)node.Profile, objEnqueue.IsInConflict);
                            int _initialUserRID = ((AssortmentFilterProfile)node.Profile).UserRID;
                            int _initialOwnerRID = ((AssortmentFilterProfile)node.Profile).OwnerUserRID;
                            ...
                            //filterProperties = new frmFilterBuilder(SAB, EAB, isReadOnly, executeAfterEditing);

                            filterProperties = SharedRoutines.GetFilterFormForExistingFilter(filterProf.Key, SAB, EAB, isReadOnly, _executeAfterEditing);
        ...
        public void EditThisFilter(int filterRID)
        {
            MIDFilterNode node = (MIDFilterNode)ItemNodeHash[filterRID];

            if (node != null)
            {
                _executeAfterEditing = true;
                try
                {
                    Cursor.Current = Cursors.WaitCursor;
                    EditNode(node);
        ...
        {
            return aClipboardDataType == eProfileType.FilterAssortment;
        }
                ...
                if (aChangedNode != null)
                {
                    ...
                    foreach (MIDFilterNode node in aStartNode.Nodes)
                    {
                        if (node.Profile.Key == aChangedNode.Profile.Key && node.Profile.ProfileType == aChangedNode.Profile.ProfileType)
                        {
                            ...
                            if (node.Profile.ProfileType != eProfileType.FilterAssortment)
                            {
                        ...
                        else if (node.NodeProfileType == eProfileType.FilterAssortmentSubFolder || node.isFolderShortcut)
                        {
        ...
        override public bool isDropAllowed(DragDropEffects aDropAction, MIDTreeNode aDropNode, ClipboardListBase aSelectedNodes)
        {
            try
            {
                if (aSelectedNodes.ClipboardDataType == eProfileType.FilterAssortment ||
                    aSelectedNodes.ClipboardDataType == eProfileType.FilterAssortmentSubFolder)
                {
        ...
        public void DeleteShortcuts(MIDTreeNode aStartNode, MIDTreeNode aDeleteNode)
        {
            ...
            try
            {
                ...
                foreach (MIDFilterNode node in deleteList)
                {
                    ...
                    else if (node.NodeProfileType == eProfileType.FilterAssortmentSubFolder ||
                        node.isFolderShortcut ||
                        (node.isChildShortcut && node.Profile.ProfileType == eProfileType.FilterAssortmentSubFolder))
                    {
                ...
                foreach (DataRow row in folderItemList)
                {
                    switch ((eProfileType)Convert.ToInt32(row["CHILD_ITEM_TYPE"]))
                    {
                        ...
                        case eProfileType.FilterAssortment:
                            ...
                            // End TT#437-MD - JSmith - Client application will not start due to a missing item on an explorer (Null reference on Client App startup.)
                            filterProf = new AssortmentFilterProfile(itemRow);
        ...
        public MIDFilterNode BuildFilterNode(AssortmentFilterProfile prof, MIDTreeNode aParentNode, int aUserId)
        {
            MIDFilterNode newNode;
        ...
        private GenericEnqueue EnqueueObject(AssortmentFilterProfile aFilterProf, bool aAllowReadOnly)
        {
            GenericEnqueue objEnqueue;
            string errMsg;
        ...
        private MIDFilterNode MoveFilterNode(MIDFilterNode aFromNode, MIDFilterNode aToNode)
        {
            ...
            try
            {
                ...
                else if (aFromNode.NodeProfileType == eProfileType.FilterAssortmentSubFolder)
                {
                    ...
                    if (aToNode.GetTopSourceNode() != aFromNode.GetTopSourceNode())
                    {
                        ...
                        folderProf.Name = FindNewFolderName(folderProf.Name, aToNode.UserId, aToNode.Profile.Key, eProfileType.FilterAssortmentSubFolder);
                ...
                else if (aFromNode.NodeProfileType == eProfileType.FilterAssortment)
                {
                    filterProf = (AssortmentFilterProfile)aFromNode.Profile;
        ...
        private MIDFilterNode JustMoveFilterNodeAndDoNotRename(MIDFilterNode aFromNode, MIDFilterNode aToNode)
        {
            ...
            try
            {
                ...
                if (aFromNode.NodeProfileType == eProfileType.FilterAssortment)
                {
                    filterProf = (AssortmentFilterProfile)aFromNode.Profile;
        ...
        private MIDFilterNode CopyFilterNode(MIDFilterNode aFromNode, MIDFilterNode aToNode, bool aFindUniqueName)
        {
            ...
            try
            {
                if (aFromNode.isSubFolder)
                {
                    ...
                    try
                    {
                        folderProf.Key = _dlFolder.Folder_Create(folderProf.UserRID, folderProf.Name, eProfileType.FilterAssortmentSubFolder);
                        _dlFolder.Folder_Item_Insert(aToNode.Profile.Key, folderProf.Key, eProfileType.FilterAssortmentSubFolder);
                ...
                {
                    filterProf = (AssortmentFilterProfile)((AssortmentFilterProfile)aFromNode.Profile).Clone();
                    filterProf.UserRID = aToNode.UserId;
                    filterProf.OwnerUserRID = aToNode.UserId;
                    ...
                    try
                    {
                        ...
                        //Begin TT#1376-MD -jsobek -Copy/Paste of Header Filter does not create Stored Procedure 
                        //filter f = new filter(filterProf.UserRID, filterProf.OwnerUserRID);
                        //f.filterName = filterProf.Name;
                        //f.filterRID = filterProf.Key;
                        filter f = filterDataHelper.LoadExistingFilter(filterProf.Key); 
                        ...
                        this.EAB.AssortmentWorkspaceExplorer.BindFilterComboBox();
                        //End TT#1376-MD -jsobek -Copy/Paste of Header Filter does not create Stored Procedure 
            ...
            try
            {
                if (aNode.isObject)
                {
                    ...
                    try
                    {
                        ...
                        //try
                        //{
                        //    _dlFolder.Folder_Item_Delete(aNode.Profile.Key, eProfileType.FilterAssortment);
                        //    _dlFolder.Folder_Shortcut_DeleteAll(aNode.Profile.Key, eProfileType.FilterAssortment);
                        //    _dlFilters.FilterDelete(aNode.Profile.Key);
                        //    _dlFilters.RemoveFilterProcedure(aNode.Profile.Key, filterTypes.AssortmentFilter);
                        ...
                        try
                        {
                            _dlFolder.Folder_Item_Delete(aNode.Profile.Key, eProfileType.FilterAssortment);	// TT#1360-MD - stodd - Convert Dropdowns on Assortment Workspace to be MID Enhanced Dropdowns
                            _dlFolder.Folder_Shortcut_DeleteAll(aNode.Profile.Key, eProfileType.FilterAssortment); // TT#1360-MD - stodd - Convert Dropdowns on Assortment Workspace to be MID Enhanced Dropdowns
                            _dlFolder.CommitData();
                        }
                        catch (DatabaseForeignKeyViolation)
                        {
                        ...
                        try
                        {
                            ...
                            // Removes user workspace filter records that are using the filter
                            _dlFilters.WorkspaceCurrentFilter_DeleteAll(aNode.Profile.Key);
                        ...
                        try
                        {
                            _dlFilters.RemoveFilterProcedure(aNode.Profile.Key, filterTypes.AssortmentFilter);	// TT#1360-MD - stodd - Convert Dropdowns on Assortment Workspace to be MID Enhanced Dropdowns
                        }
                        catch (DatabaseForeignKeyViolation)
                        {
                            MessageBox.Show(SAB.ClientServerSession.Audit.GetText(eMIDTextCode.msg_DeleteFailedDataInUse), Text, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        ...
                        EAB.AssortmentWorkspaceExplorer.BindFilterComboBox();
                ...
                else if (aNode.isSubFolder)
                {
                    ...
                    if (aNode.Nodes.Count == 0)
                    {
                        ...
                        try
                        {
                            _dlFolder.Folder_Item_Delete(aNode.Profile.Key, eProfileType.FilterAssortmentSubFolder);
                            _dlFolder.Folder_Delete(aNode.Profile.Key, eProfileType.FilterAssortmentSubFolder);
                            _dlFolder.CommitData();
                        }
                        catch (Exception exc)
                        {
                ...
                else if (aNode.isObjectShortcut)
                {
                    ...
                    try
                    {
                        _dlFolder.Folder_Shortcut_Delete(aNode.ParentId, aNode.Profile.Key, eProfileType.FilterAssortment);
                        _dlFolder.CommitData();
                    }
                    catch (DatabaseForeignKeyViolation)
                    {
                ...
                else if (aNode.isFolderShortcut)
                {
                    ...
                    try
                    {
                        _dlFolder.Folder_Shortcut_Delete(aNode.ParentId, aNode.Profile.Key, eProfileType.FilterAssortmentSubFolder);
                        _dlFolder.CommitData();
                    }
                    catch (DatabaseForeignKeyViolation)
                    {
        ...
        // End TT#5437 - JSmith - Copying Global Header Folder w filters attached to My filters get Null error  
        {
            ...
            try
            {
                ...
                if (node == null)
                {
                    ...
                    try
                    {
                        secAdmin.DeleteUserItemByTypeAndRID((int)eProfileType.FilterAssortment, f.filterRID);
                        secAdmin.AddUserItem(userRID, (int)eProfileType.FilterAssortment, f.filterRID, ownerRID);
                        secAdmin.CommitData();
                    }
                    catch (Exception exc)
                    {
                    ...
                    try
                    {
                        _dlFolder.Folder_Item_Insert(parentNode.NodeRID, f.filterRID, eProfileType.FilterAssortment);
                        _dlFolder.CommitData();
                    }
                    catch (Exception exc)
                    {
                    ...
                    AssortmentFilterProfile prof = new AssortmentFilterProfile(f.filterRID);
                ...
                else
                {
                    ...
                    AssortmentFilterProfile prof = (AssortmentFilterProfile)node.Profile;
                    prof.Name = f.filterName;
                    prof.IsLimited = f.isLimited;
                    prof.ResultLimit = (int)f.resultLimit;
