...
{
	/// <summary>
	/// Summary description for StockModifierModelMaint.
	/// </summary>
	public class frmStockModifierModelMaint : ModelFormBase
	{
        //		private SessionAddressBlock _SAB; //TT#1638 - MD - Revised Model Save - RBeck
...
//		private CalendarDateSelector _frmCalDtSelector;
        private ProfileList _stkModModelProfileList;
        //private Infragistics.Win.UltraWinGrid.UltraGrid ugModel;
        //private System.Windows.Forms.Label lblModelName;
		private System.Windows.Forms.TextBox txtStockModifierDefault;
		private System.Windows.Forms.Label lblStockModifierDefault;
		...
		public frmStockModifierModelMaint(SessionAddressBlock aSAB) : base (aSAB)
		{
			try
			{
				//
				...
				LoadModelComboBox();
                SetText();     
				FunctionSecurity = SAB.ClientServerSession.GetMyUserFunctionSecurityAssignment(eSecurityFunctions.AdminModelsStockModifier);
				DetermineSecurity();
...
//End Track #3707 - JScott - Null Reference on model save
		...
		protected override void Dispose( bool disposing )
		{
			try
			{
				if( disposing )
				{
					...
					this.txtStockModifierDefault.TextChanged -= new System.EventHandler(this.txtStockModifierDefault_TextChanged);
					this.txtStockModifierDefault.Leave -= new System.EventHandler(this.txtStockModifierDefault_Leave);
				}
		...
		private void InitializeComponent()
		{
            ...
            this.txtStockModifierDefault = new System.Windows.Forms.TextBox();
            this.lblStockModifierDefault = new System.Windows.Forms.Label();
            ...
            // 
            // txtStockModifierDefault
            // 
            this.txtStockModifierDefault.Location = new System.Drawing.Point(320, 56);
            this.txtStockModifierDefault.Name = "txtStockModifierDefault";
            this.txtStockModifierDefault.Size = new System.Drawing.Size(79, 20);
            this.txtStockModifierDefault.TabIndex = 30;
            this.txtStockModifierDefault.TextAlign = System.Windows.Forms.HorizontalAlignment.Right;
            this.txtStockModifierDefault.TextChanged += new System.EventHandler(this.txtStockModifierDefault_TextChanged);
            this.txtStockModifierDefault.Leave += new System.EventHandler(this.txtStockModifierDefault_Leave);
            // 
            // lblStockModifierDefault
            // 
            this.lblStockModifierDefault.Location = new System.Drawing.Point(190, 55);
            this.lblStockModifierDefault.Name = "lblStockModifierDefault";
            this.lblStockModifierDefault.Size = new System.Drawing.Size(120, 26);
            this.lblStockModifierDefault.TabIndex = 29;
            this.lblStockModifierDefault.Text = "Default Value:";
            this.lblStockModifierDefault.TextAlign = System.Drawing.ContentAlignment.MiddleRight;
            // 
            // frmStockModifierModelMaint
            // 
            this.ClientSize = new System.Drawing.Size(720, 517);
            this.Controls.Add(this.txtStockModifierDefault);
            this.Controls.Add(this.lblStockModifierDefault);
            this.Name = "frmStockModifierModelMaint";
            this.Text = "Stock Modifier Model Maintenance";
            ...
            this.Controls.SetChildIndex(this.lblStockModifierDefault, 0);
            this.Controls.SetChildIndex(this.txtStockModifierDefault, 0);
		...
		private void ugModel_InitializeLayout(object sender, Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs e)
		{
			try
			{
				...
				InfragisticsLayout layout = layoutData.InfragisticsLayout_Read(SAB.ClientServerSession.UserRID, eLayoutID.stockModifierModelGrid);
		...
		override public void InitializeForm()
		{
			try
			{
				...
				if (_modelLocked)
				{
					SAB.HierarchyServerSession.DequeueModel(eModelType.StockModifier, _modelRID);
					_modelLocked = false;
				}
				_tableLoaded = false;
				DoValueByValueEdits = false;
				this.txtStockModifierDefault.Text = "1.0";
...
//Begin TT#1638 - Revised Model Save - RBeck
				...
				Format_Title(eDataState.New, eMIDTextCode.frm_StockModifierModel, null);
				SetReadOnly(true);
				DetermineSecurity();
		...
		override public bool InitializeForm(string currModel)
		{
			...
			try
			{
				...
				if (currModel == "")
				{
                    ...
                    btnDelete.Enabled = false;
					if (_stkModModelProfileList.Count > 0)
					{
						displayModel = false;
				...
				StkModModelProfile smmp = SAB.HierarchyServerSession.GetStkModModelData(currModel);
				if (displayModel)
				{
					if (!_newModel && FunctionSecurity.AllowUpdate)
					{
						smmp = (StkModModelProfile) SAB.HierarchyServerSession.GetModelDataForUpdate(eModelType.StockModifier, smmp.Key, true);
					...
					if (initializeSuccessful)
					{
						...
						Format_Title(dataState, eMIDTextCode.frm_StockModifierModel, smmp.ModelID);
						_modelRID = smmp.Key;
						_currModel = smmp.ModelID;
						this.txtStockModifierDefault.Text = smmp.StkModModelDefault.ToString(CultureInfo.CurrentUICulture);
						_gridDataTable.Rows.Clear();
						foreach (StkModModelEntry smme in smmp.ModelEntries)
						{
							//Begin TT#169 - JSmith - enter max of 1000 and receive 'must be integer'
                            //_gridDataTable.Rows.Add(new object[] {smme.StkModModelEntryValue.ToString(CultureInfo.CurrentUICulture), smme.DateRange, smme.DateRange.DisplayDate});
                            _gridDataTable.Rows.Add(new object[] { smme.StkModModelEntryValue, smme.DateRange, smme.DateRange.DisplayDate });
                            //End TT#169
						}
						this.ugModel.DataSource = _gridDataTable;
						...
						foreach(  UltraGridRow gridRow in ugModel.Rows )
						{
							...
							StkModModelEntry smme = (StkModModelEntry)smmp.ModelEntries[index];
				...
				else
				{
					...
					Format_Title(dataState, eMIDTextCode.frm_StockModifierModel, "");
					SetReadOnly(FunctionSecurity.AllowUpdate);  //Security changes - 1/24/2005 vg
				}
				if (currModel == "")
				{
		...
		{
			this.Text = MIDText.GetTextOnly(eMIDTextCode.frm_StockModifierModel);
		}
...
//Begin     TT#1638 - MD - Revised Model Save - RBeck
        ...
        //{
        //    _stkModModelProfileList = _SAB.HierarchyServerSession.GetStkModModels();
        //    cbModelName.Items.Clear();

        //    foreach (ModelName modelName in _stkModModelProfileList.ArrayList)
        //    {
...
//End      TT#1638 - MD - Revised Model Save - RBeck
			...
			try
			{
				...
				if (_modelRID != Include.NoRID)
				{
				    ...
				    //BEGIN TT#601-MD-VStuart-FWOS Model attempts delete while InUse
				    {
                        ...
                        var emp = new StkModModelProfile(_modelRID);
				        ...
				        {
				            Cursor = Cursors.WaitCursor;
				            StkModModelProfile smmp = new StkModModelProfile(_modelRID);
				...
				if (itemDeleted)
				{
					if (_stkModModelProfileList.Count > 0)
					{
						...
						string modelName = ((ModelName)_stkModModelProfileList[nextItem]).ModelID;
        ...
        protected override DataTable GetFilteredModels(string stockModifierNameFilter, bool isCaseSensitive)
        {
            try
            {
                MerchandiseHierarchyData modelsData = new MerchandiseHierarchyData();

                return modelsData.GetFilteredStockModifierModels(stockModifierNameFilter, isCaseSensitive);
            }
            catch (Exception ex)
            {
...
//Begin     TT#1638 - MD - Revised Model Save - RBeck
         protected override void ModelPreSave()
        {
           ...
           smmp = new StkModModelProfile(_modelRID);
            ...
            if (this.txtStockModifierDefault.Text.Length > 0)
            {
                if (!DefaultValueValid(txtStockModifierDefault.Text, ref errorMessage))
                {
                ...
                {
                    smmp.StkModModelDefault = Convert.ToDouble(this.txtStockModifierDefault.Text, CultureInfo.CurrentUICulture);
                }
            }
            else
            {
                smmp.StkModModelDefault = 0;
            }
            ...
            foreach (UltraGridRow gridRow in ugModel.Rows)
            {
                ...
                {
                    modelEntry.StkModModelEntryValue = Convert.ToDouble(gridRow.Cells["Value"].Value, CultureInfo.CurrentUICulture);
                }
        ...
        //        //bool saveAsCanceled = false;
                ...
                //    DoValueByValueEdits = false;
                    ...
                    //        if (!saveAsCanceled)
                    //        {
                    //            StkModModelProfile checkExists = _SAB.HierarchyServerSession.GetStkModModelData(formSaveAs.SaveAsName);
                    ...
                    //                    continueSave = true;
                    //                    smmp.ModelChangeType = eChangeType.update;
                    //                    StkModModelProfile smmp2 =  _SAB.HierarchyServerSession.GetStkModModelData(formSaveAs.SaveAsName);
                    //                    _modelRID = smmp2.Key;
                    //                    _saveAsName = formSaveAs.SaveAsName;
                    //                }
                    //            }
                    ...
                    //    smmp.Key = _modelRID;
                    //    saveKey = _modelRID;
                    //    _modelRID = _SAB.HierarchyServerSession.StkModModelUpdate(smmp);
                    //    _changeMade = true;
                    //    ChangePending = false;
                    //    btnSave.Enabled = false;
                    //    //btnSaveAs.Enabled = false;
                    ...
                    //        {
                    //            _SAB.HierarchyServerSession.DequeueModel(eModelType.StockModifier, saveKey);
                    //            _modelLocked = false;
                    //        }
...
////						{
////							string modelName = ((ModelName)(_stkModModelProfileList[0])).ModelID;
////							InitializeForm(modelName);
////						}
////						else
////						{
...
//                    {
//                        this.txtStockModifierDefault.SelectionStart = 0;
//                        this.txtStockModifierDefault.SelectionLength = this.txtStockModifierDefault.Text.Length;
//                        this.txtStockModifierDefault.Select();
//                        this.txtStockModifierDefault.Focus();
//                    }
//                    else
//                    {
...
//            return true;
        ...
        protected override ProfileList GetModels()
         {
             _stkModModelProfileList = SAB.HierarchyServerSession.GetStkModModels();
             return _stkModModelProfileList;
         }
        ...
        protected override void GetModelRID(string _saveAsName, ref int _modelRID)
        {
            smmp.ModelID = _saveAsName;
            smmp.Key = _modelRID;
            _modelRID = SAB.HierarchyServerSession.StkModModelUpdate(smmp);
        }
        ...
        {            
            StkModModelProfile checkExists = SAB.HierarchyServerSession.GetStkModModelData(SaveAsName);
            return checkExists.Key;
        }
        //protected override string GetModelName(int _modelIndex)
        //{
        //    string modelName = ((ModelName)(_stkModModelProfileList[_modelIndex])).ModelID;
        //    return modelName;
        //}
...
//End     TT#1638 - MD - Revised Model Save - RBeck
		override protected void BeforeClosing()
		{
			try
			{
				if (FunctionSecurity.AllowUpdate)
				{
					...
					{
						InfragisticsLayoutData layoutData = new InfragisticsLayoutData();
						layoutData.InfragisticsLayout_Save(SAB.ClientServerSession.UserRID, eLayoutID.stockModifierModelGrid, ugModel);
					}
				}
		...
		override protected void AfterClosing()
		{
			...
			{
				HandleException(ex, "ftmStockModifierModelMaint.AfterClosing");
			}
		}
...
//End     TT#1638 - MD - Revised Model Save - RBeck
		...
		private void txtStockModifierDefault_Leave(object sender, System.EventArgs e)
		{
			try
			{
				string errorMessage = null;
		...
		private bool DefaultValueValid(string defaultValue, ref string errorMessage)
		{
			...
			if (ErrorFound)
			{
				ErrorProvider.SetError (this.txtStockModifierDefault,errorMessage);
				return false;
			}
			else
			{
				ErrorProvider.SetError (this.txtStockModifierDefault,"");
				return true;
			}
		}
...
//End     TT#1638 - MD - Revised Model Save - RBeck	
		...
		private void txtStockModifierDefault_TextChanged(object sender, System.EventArgs e)
		{
			if (FormLoaded && _tableLoaded)
			{
				ChangePending = true;
