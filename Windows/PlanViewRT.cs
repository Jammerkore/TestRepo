...
{
	public partial class PlanViewRT : MIDFormBase, IFormBase
    {
...
//End Track #5244 - JScott - Add debugging for error encountered in PlanView window
        ...
        public PlanViewRT(SessionAddressBlock sab, PlanOpenParms aOpenParms)
            : base(sab)
        {
            try
            {
        ...
        private void PlanView_Load(object sender, System.EventArgs e)
        {
            try
            {
                ...
                // Begin TT#639-MD - RMatelic -Add option on OTS Forecast Reviews to have Totals on the Right instead of the Left of the details
                spcVLevel2_DoubleClick(spcVLevel2, null);
                // TT#639-MD
       ...
       // }
        ...
        private void CalcColSplitPosition3(bool aOverrideLock)
        {
            ...
            //try
            {
                ...
                if (((PagingGridTag)g3.Tag).Visible)
                {
                    ...
                    {
                        // Begin TT#639 - Agallagher 
                        _currColSplitPosition3 = g5.Width - _currColSplitPosition2;
                        // End TT#639 - AGallagher
                       
                        // Begin TT#639-MD - RMatelic -Add option on OTS Forecast Reviews to have Totals on the Right instead of the Left of the details
                        if (!_formLoading)
                        {
                            // Begin TT#4004 - JSmith - Chain Totals Right fails with index out of range error
                            //_currColSplitPosition3 = spcVLevel2.Width - g3.Cols[1].Right + 11;
                            if (g2.Cols.Count > 1)
                            {
                                _currColSplitPosition3 = spcVLevel2.Width - g2.Cols[1].Right + 11;
                            }
                            else if (g2.Cols.Count == 1)
                            {
                                _currColSplitPosition3 = spcVLevel2.Width - g2.Cols[0].Right - g2.Cols[0].Width + 11;
                            }
                            else
                            {
                                _currColSplitPosition3 = spcVLevel2.Width + 11;
                            }
                            // End TT#4004 - JSmith - Chain Totals Right fails with index out of range error
                        }
                        // End TT#639
                    }
                }
                ...
                //        else if (_columnGroupedBy == GroupedBy.GroupedByTime)
                //        {
                //            lastKey = ((ColumnHeaderTag)g3.Cols[g3.LeftCol].UserData).DetailRowColHeader.Profile.Key;

                //            for (i = g3.LeftCol + 1; i < g3.Cols.Count; i++)
                //            {
                //                if (((ColumnHeaderTag)g3.Cols[i].UserData).DetailRowColHeader.Profile.Key != lastKey)
                //                {
                //                    break;
                //                }
                //            }
                ...
                //        else
                //        {
                //            lastKey = ((ColumnHeaderTag)g3.Cols[g3.LeftCol].UserData).GroupRowColHeader.Profile.Key;

                //            for (i = g3.LeftCol + 1; i < g3.Cols.Count; i++)
                //            {
                //                if (((ColumnHeaderTag)g3.Cols[i].UserData).GroupRowColHeader.Profile.Key != lastKey)
                //                {
                //                    break;
                //                }
                //            }
                ...
                //        _currColSplitPosition3 = (g3.Cols[i - 1].Right - g3.Cols[g3.LeftCol].Left) + 2;
                //    }
                //}
                //else
                //{
			...
			try
			{
				...
				if (exportG4.Rows.Count > 0)
				{
					...
					for (i = 0; i < exportG4.Rows.Count; i++)
					{
                        ...
                        // add total columns
                        // Begin TT#1590-MD - JSmith - Exporting Forecast Store Review only exports first column
                        //else if (totalPlanWaferCellTable != null)
                        else if (detailPlanWaferCellTable != null)
                        // End TT#1590-MD - JSmith - Exporting Forecast Store Review only exports first column
                        {
        ...
        private void g2_VisibleChanged(object sender, System.EventArgs e)
        {
            try
            {
                // Begin TT#1493-MD - JSmith - Right Totals with a single week returns no details
                //if (((PagingGridTag)g2.Tag).Visible)
                if (((PagingGridTag)g3.Tag).Visible)
                // End TT#1493-MD - JSmith - Right Totals with a single week returns no details
                {
...
// End Track #4868
        ...
        private void ReformatRowsChanged(bool aClearGrid)
        {
            try
            {
                ...
                SetScrollBarPosition(vScrollBar4, vScrollBar4.Value);
                // Begin TT#639-MD - RMatelic -Add option on OTS Forecast Reviews to have Totals on the Right instead of the Left of the details
                if (!((SplitterTag)spcVLevel2.Tag).Locked)
                {
                     spcVLevel2_DoubleClick(spcVLevel2, null);
                }
                else
