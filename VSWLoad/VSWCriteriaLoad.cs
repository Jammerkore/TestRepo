...
{
    /// <summary>
    /// Summary description for VSWLoadCriteriaLoad.
    /// </summary>
    class VSWLoadCriteriaLoad
    {
        /// <summary>
        ...
        static int Main(string[] args)
        {
            ...
            string eventLogID = "MIDVSWCriteriaLoad";
            string sourceModule = "VSWCriteriaLoad.cs";
            ...
            try
            {
                ...
                try
                {
                    ...
                    authentication = SAB.ClientServerSession.UserLogin(userId, passWd, eProcesses.VSWCriteriaLoad);
                    ...
                    if (fileLocation == null || fileLocation == "")
                    {
                        ...
                        msgText += "VSW Load process NOT run";
                    ...
                    else
                    {
                        if (!File.Exists(fileLocation))
                        {
                            ...
                            {
                                SAB.ClientServerSession.Audit.Add_Msg(eMIDMessageLevel.Severe, "VSW Load process failed", sourceModule);
                        ...
                        else
                        {
                            ...
                            if (txnFileInfo.Length == 0)
                            {
                                ...
                                {
                                    SAB.ClientServerSession.Audit.Add_Msg(eMIDMessageLevel.Severe, "VSW Load failed", sourceModule);
                                    SAB.ClientServerSession.Audit.UpdateHeader(eProcessCompletionStatus.Failed, eMIDTextCode.sum_Failed, "", eMIDMessageLevel.Severe);
                                }
                                highestMessage = eMIDMessageLevel.None;
                                try
                            ...
                            else
                            {
                                ...
                                {
                                    errorFound = SAB.HierarchyServerSession.LoadXML_IMOFile(SAB, fileLocation, delimiter, ref errorFound);
                                    //dpclp.ProcessXMLTransFile(fileLocation, delimiter, ref errorFound);
                                }
                                else
                                {
                                    errorFound = SAB.HierarchyServerSession.LoadDelimitedIMOFile(SAB, fileLocation, delimiter, ref errorFound);
