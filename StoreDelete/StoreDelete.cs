...
namespace MIDRetail.StoreDelete
{
	class StoreDelete
	{
		static int Main(string[] args)
		{
			...
			return deleteStore.Process(args);
		}
		...
		public class DeleteStoreWorker
		{
			...
			ConsoleStatusBar _statusBar = new ConsoleStatusBar();
			...
			public int Process(string[] args)
			{
				...
				try
				{
					...
					if (AreServicesStopped())
					{
						...
						{
							LogError("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
							WriteEventLog("No stores are marked for deletion.", EventLogEntryType.Error);
							LogError("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
							// Switch to AnalysisOnly mode so processwon't continue
							_analysisOnly = true;
							errorFound = true;
						...
						if (!_analysisOnly && !errorFound)
						{
							try
							{
								...
								if (!errorFound)
								{
									errorFound = DeleteStoresConcurrent();
								}
							}
							catch
							{
							...
							finally
							{
								...
								if (errorFound)
								{
									_statusBar.RenderConsoleProgress(100, '\u2591', ConsoleColor.Red, "");
									Console.WriteLine(" Error during Store Delete.");
								}
								else
								{
									_statusBar.RenderConsoleProgress(100, '\u2592', ConsoleColor.Green, "100% Complete");
									Console.WriteLine(" Store Delete was Successful.");
								}
							}
						}
					}
					...
					{
						errorFound = true;
						WriteEventLog("MIDRetail Services must be down for Store Delete to process. Please shut down services and restart Store Delete.", EventLogEntryType.Error);
					}
				}
				...
				foreach (DataRow row in analysisTable.Rows)
				{
					if (row["ANALYSIS_DATE"] != DBNull.Value)
					{
						...
						if (AnalysisDate < sevenDaysAgo)
						{
							...
							LogError("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
							//LogError("The system indicates that a Store Delete is already running. Only one Store Delete process may be run at a time.");
							WriteEventLog("The system indicates that the Delete Store Analysis was run over 7 days ago. Please rerun analysis or change the 'SkipAnalysis' setting to false.", EventLogEntryType.Error);
							LogError("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
							errorFound = true;
							break;
						}
			...
			private bool DeleteStoresConcurrent()
			{
				bool errorFound = false;
				...
				try
				{
					...
					DeleteStoreFromTable("IN_USE_FILTER_XREF", _batchSize);
					...
					Stack storeRemovalStack = BuildStack(storeRemovalArray, removalList);
					Stack storeCopyStack = BuildStack(storeCopyArray, copyList);
					...
					Stack setRemovalStack = BuildStack(storeRemovalArray, removalSetList);
					Stack setCopyStack = BuildStack(storeCopyArray, copySetList);
					//======================
					// Do Set COPYs
					//======================
					...
					totalTimer.Stop();
					LogStoreMessage("COMPLETED", " Total_Deleted", totalRowsDeleted, totalTimer.ElaspedTime);
			...
			private Stack BuildStack(ArrayList stackList, List<string> procList)
			{
				Stack aStack = new Stack();

				foreach (string proc in procList)
				{
					StoreDeleteProcess aProcess = new StoreDeleteProcess(_connectionString, _SAB, proc, _storeData, _log, _batchSize, _maxTableNameLength, _maxStoreIdLength);
	...
	public class StoreDeleteProcess : ConcurrentProcess
	{
		...
		public StoreDeleteProcess(string connectionString, SessionAddressBlock aSAB, string proc,  StoreData storeData, log4net.ILog log, int batchSize, int maxTableNameLength, int maxStoreIdLength)
			: base(null)
		{
			try
			{
			...
			catch (Exception exc)
			{
				...
				LogError("Exception encountered in creating stack for procedure: " + proc);
				throw;
			}
		}
		...
		override public void ExecuteProcess()
		{
			try
			{
				if (_isCopyLoad)
				{
					LogInfo(_tableName + " Copy/Load Started");
				}
				else
				{
					LogInfo(_tableName + " Delete Started");
				}
			...
			finally
			{
				...
				if (_isCopyLoad)
				{
					...
					_storeData.TruncateTable(_tableNameCopy);
					_storeData.DropTable(_tableNameCopy);
					_storeData.CommitData();
				}
