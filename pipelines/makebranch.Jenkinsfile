pipeline
{
	...
	stages
	{
		stage('Make Branch')
		{
			...
			{
				sh '''
					git checkout -b "rel/$BRANCHNAME" || true
					git push https://$GITAUTH@$GITREPO "rel/$BRANCHNAME" || true
				'''
			}
		}
	}
	post
	{
		success
		{
			emailext body: "<h2>rel/$BRANCHNAME</h2><p>The branch rel/$BRANCHNAME has been created in repo https://$GITREPO.</p><p>Please make all the necessary adjustments. Git commit: $COMMITID</p><p>Items to consider adjusting:<ul><li>Adding branch to be build on commit</li><li>Setting SETUP script for branch</li><li>Preparing Jenkins pipelines for branch</li></ul></p>", subject: "Branch rel/$BRANCHNAME created", to: "$EMAIL"
