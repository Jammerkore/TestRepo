...
{
    public class taskStoreDailyBins_Step3_UpsertClass
    {

        private SubTaskSelectControl _task;
        private SqlCommand _cmd;

        public taskStoreDailyBins_Step3_UpsertClass(SubTaskSelectControl task, SqlCommand cmd)
        {
            _task = task;
            _cmd = cmd;
        }
        ...
        public void Upsert()
        {
            try
            {
                ...
                if (!_formClosing)
                {
                    // Now that the rows are in the temporary work table - use the work table and update/insert into the daily history normalized tables via a stored procedure call
                    _task.UpdateStatus("Updating");

                    _cmd.CommandText = "[dbo].[CONVERT_STORE_DAILY_BINS]";
