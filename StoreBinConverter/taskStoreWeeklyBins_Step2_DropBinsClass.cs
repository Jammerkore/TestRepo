...
{
    public class taskStoreWeeklyBins_Step2_DropBinsClass
    {
        private SubTaskSelectControl _task;
        private SqlCommand _cmd;

        public taskStoreWeeklyBins_Step2_DropBinsClass(SubTaskSelectControl task, SqlCommand cmd)
        {
            _task = task;
            _cmd = cmd;
        }
        ...
        public void DropBins()
        {
            try
            {
                ...
                if (!_formClosing)
                {
                    _cmd.CommandText = "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[STORE_WEEK_HISTORY_BIN]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) TRUNCATE TABLE STORE_WEEK_HISTORY_BIN";
                ...
                if (!_formClosing)
                {
                    _cmd.CommandText = "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[STORE_WEEK_HISTORY_BIN]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) DROP TABLE STORE_WEEK_HISTORY_BIN";
                    _cmd.CommandType = CommandType.Text;
                    _cmd.ExecuteNonQuery();
                    _task.UpdateTime();
                }
