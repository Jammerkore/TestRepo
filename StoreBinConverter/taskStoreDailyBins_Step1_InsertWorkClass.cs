...
{
    public class taskStoreDailyBins_Step1_InsertWorkClass
    {
        private SubTaskSelectControl _task;
        private SqlCommand _cmd;
        private int[] _storeRIDs;
        private SqlBulkCopy bulkCopyDay;
        private string _databaseConnectionString;
        private long totalDailyRows = 0;
        ...
        public taskStoreDailyBins_Step1_InsertWorkClass(SubTaskSelectControl task, 
            SqlCommand cmd, 
        ...
        public void InsertWork()
        {
            try
            {
                ...
                _cmd.CommandText = "SELECT COUNT(*) FROM STORE_DAY_HISTORY_BIN";
                ...
                if (totDailyRows != null && totDailyRows != DBNull.Value)
                {
                    ...
                    long.TryParse(stemp, out totalDailyRows);  
                }

                if (totalDailyRows == -1)
                {
                    _task.UpdateStatus("Failed");
                    _task.UpdateLog(_task.ultraCheckEditor1.Text + " - Error: Could not obtain total row count.");
                    return;
                }
                _task.UpdateTime();
                ...
                if (totalDailyRows == 0)
                {
                    _task.UpdateLog(System.Environment.NewLine + "Zero store day bin rows to process");
                ...
                _cmd.CommandText = "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[STORE_HISTORY_WORK_DAY]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) drop table [dbo].[STORE_HISTORY_WORK_DAY]";
                _cmd.CommandType = CommandType.Text;
                ...
                DataTable dtWork = new DataTable("STORE_HISTORY_WORK_DAY");
                ...
                long totalRowsToProcess = totalDailyRows * _storeRIDs.LongLength;
                _task.ultraProgressBar1.Maximum = (int)(totalRowsToProcess / 1000) + 1000; //adding an extra 1000 for rounding

                _task.UpdateLog(System.Environment.NewLine + "Processing " + totalRowsToProcess.ToString("###,###,###,###,###,###,##0") + " total store day bin rows.");

                _task.UpdateStatus("Processing");


                // Setup the SqlBulkCopy
                bulkCopyDay = new SqlBulkCopy(_databaseConnectionString, SqlBulkCopyOptions.TableLock);
                int idealBatchSizeFactor = (5000 / _storeRIDs.Length) + 1; //we want to process about 5000 rows at a time
                bulkCopyDay.BatchSize = (_storeRIDs.Length * idealBatchSizeFactor); // 5000;
                bulkCopyDay.NotifyAfter = 0; // _storeRIDs.Length; // 1000;
                //bulkCopyDay.SqlRowsCopied += new SqlRowsCopiedEventHandler(_task.bulkCopy_SqlRowsCopied);
                bulkCopyDay.DestinationTableName = "STORE_HISTORY_WORK_DAY";

                ReadBinDataAndWriteToTempWorkTable(mimimumTime, maximumTime, dtWork, ref bulkCopyDay);
        ...
        private string MakeWorkTableSQL()
        {
            ...
            sb.AppendLine("CREATE TABLE [dbo].[STORE_HISTORY_WORK_DAY](");
        ...
        private string MakeWorkProcedureSQL(int numberOfHistoryTables)
        {
            ...
            sb.AppendLine("-- =============================================");
            sb.AppendLine("CREATE PROCEDURE [dbo].[CONVERT_STORE_DAILY_BINS] ");
            sb.AppendLine("AS ");
            sb.AppendLine("BEGIN ");
            sb.AppendLine("");
            ...
            sb.AppendLine("	");
            sb.AppendLine("FROM [dbo].[STORE_HISTORY_WORK_DAY]  wd ");
            ...
            for (int i = 0; i < numberOfHistoryTables; i++)
            {
                ...
                sb.AppendLine("INNER JOIN [dbo].[STORE_HISTORY_DAY" + i.ToString() + "] d ON sv.HN_MOD=" + i.ToString() + " AND d.HN_RID=sv.HN_RID AND d.TIME_ID=sv.TIME_ID AND d.ST_RID=sv.ST_RID ");
                sb.AppendLine("");
            }
            ...
            sb.AppendLine("");
            sb.AppendLine("--Begin Process Step 3: Update day history tables");
            sb.AppendLine("SET @TIME_TO_EXECUTE_PROCESS_STEP = getDate();");
            ...
            for (int i=0; i < numberOfHistoryTables; i++)
            {
                ...
                sb.AppendLine("FROM [dbo].[STORE_HISTORY_DAY" + i.ToString() + "] d ");
                ...
                sb.AppendLine("INSERT INTO @PROCESS_SUMMARY SELECT 'Step 3: Update day history tables - Table " + i.ToString() + "',0,DATEDIFF(millisecond, @TIME_TO_EXECUTE, getDate()),'Updated Rows:', (SELECT @UPDATED_ROWS) ");
                sb.AppendLine("");
            }

            sb.AppendLine("");
            sb.AppendLine("INSERT INTO @PROCESS_SUMMARY SELECT 'Step 3: Update day history tables',DATEDIFF(millisecond, @TIME_TO_EXECUTE_PROCESS_STEP, getDate()),0,'Total Updated Rows:', (SELECT @TOTAL_UPDATED_ROWS) ");
            sb.AppendLine("--End Process Step 3: Update day history tables");

            sb.AppendLine("--Begin Process Step 4: Add rows to day history tables");
            sb.AppendLine("SET @TIME_TO_EXECUTE_PROCESS_STEP = getDate();");
            ...
            for (int i=0; i < numberOfHistoryTables; i++)
            {
                ...
                sb.AppendLine("SET @TIME_TO_EXECUTE = getDate();");
                sb.AppendLine("INSERT INTO [dbo].[STORE_HISTORY_DAY" + i.ToString() + "] ");
                sb.AppendLine("(");
                ...
                sb.AppendLine("INSERT INTO @PROCESS_SUMMARY SELECT 'Step 4: Add rows to day history tables - Table " + i.ToString() + "',0,DATEDIFF(millisecond, @TIME_TO_EXECUTE, getDate()),'Inserted Rows:', (SELECT @INSERTED_ROWS) ");
                sb.AppendLine("");
            }

            sb.AppendLine("");
            sb.AppendLine("INSERT INTO @PROCESS_SUMMARY SELECT 'Step 4: Add rows to day history tables',DATEDIFF(millisecond, @TIME_TO_EXECUTE_PROCESS_STEP, getDate()),0,'Total Inserted Rows:', (SELECT @TOTAL_INSERTED_ROWS)");
            sb.AppendLine("--End Process Step 4: Add rows to day history tables");
            sb.AppendLine("");
            sb.AppendLine("");
            sb.AppendLine("");
