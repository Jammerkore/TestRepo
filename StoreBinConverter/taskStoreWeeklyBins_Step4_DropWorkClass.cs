...
{
    public class taskStoreWeeklyBins_Step4_DropWorkClass
    {
        private SubTaskSelectControl _task;
        private SqlCommand _cmd;

        public taskStoreWeeklyBins_Step4_DropWorkClass(SubTaskSelectControl task, SqlCommand cmd)
        {
            _task = task;
            _cmd = cmd;
        }
        ...
        public void DropWork()
        {
            try
            {
                ...
                if (!_formClosing)
                {
                    _cmd.CommandText = "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[STORE_HISTORY_WORK_WEEK]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) TRUNCATE TABLE STORE_HISTORY_WORK_WEEK";
                ...
                if (!_formClosing)
                {
                    _cmd.CommandText = "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[STORE_HISTORY_WORK_WEEK]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) DROP TABLE STORE_HISTORY_WORK_WEEK";
                    _cmd.CommandType = CommandType.Text;
                    _cmd.ExecuteNonQuery();
                    _task.UpdateTime();
                }
