...
{
    public class taskVSW_Step4_DropWorkClass
    {
        private SubTaskSelectControl _task;
        private SqlCommand _cmd;

        public taskVSW_Step4_DropWorkClass(SubTaskSelectControl task, SqlCommand cmd)
        {
            _task = task;
            _cmd = cmd;
        }
        ...
        public void DropWork()
        {
            try
            {
                ...
                if (!_formClosing)
                {
                    _cmd.CommandText = "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[VSW_REVERSE_ON_HAND_WORK]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) TRUNCATE TABLE VSW_REVERSE_ON_HAND_WORK";
                ...
                if (!_formClosing)
                {
                    _cmd.CommandText = "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[VSW_REVERSE_ON_HAND_WORK]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) DROP TABLE VSW_REVERSE_ON_HAND_WORK";
                    _cmd.CommandType = CommandType.Text;
                    _cmd.ExecuteNonQuery();
                    _task.UpdateTime();
                }
