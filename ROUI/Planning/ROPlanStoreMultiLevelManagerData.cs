...
namespace Logility.ROUI
{
    ...
    /// </summary>
    public class ROPlanStoreMultiLevelManagerData : ROPlanStoreManagerData
    {
        ...
        public ROPlanStoreMultiLevelManagerData(SessionAddressBlock aSAB, PlanOpenParms aOpenParms)
    ...
    /// </summary>
    abstract public class ROStoreMultiLevelViewData : ROStoreViewData
    {
        public DataSet chartDataSet = null;
        public ROPlanStoreMultiLevelManagerData managerData; //this gets passed in and is set once per instance of the screen
        ...
        public ROStoreMultiLevelViewData(int viewRID, ref ROPlanStoreMultiLevelManagerData managerData)
...
//                #endif
        ...
        public void RebuildGridData()
        {
            ...
            Initialize();
            BuildGridsFromCubeData();
            //GetGrids(); this code must be added
        }
    ...
    /// Data that is defined per view
    /// </summary>
    public class ROStoreMultiLevelLadderViewData : ROStoreMultiLevelViewData
    {
        public ROStoreMultiLevelLadderViewData(int viewRID, ref ROPlanStoreMultiLevelManagerData managerData)
        ...
        override public PagingCoordinates SetPageCoordinates(int iStartingRowIndex, int iNumberOfRows, int iStartingColIndex, int iNumberOfColumns)
        {
            ...
            first = (_startingRowIndex - 1) * _numberOfRows;
            ...
            pagingCoordinates.LastRowItem = first + selectedPeriodCount;
            pagingCoordinates.TotalRowItems = periodCount;

            first = (_startingColIndex - 1) * _numberOfColumns;
            ...
            pagingCoordinates.TotalColItems = _sortedVariableHeaders.Count;
        ...
        override public ROCubeMetadata CreateMetadataFromViewData(ROCubeGetMetadataParams getMetadataParams)
        {
            eROReturnCode returnCode = eROReturnCode.Successful;
            string message = null;
            long instanceID = 0;
            eGridOrientation gridOrientation = eGridOrientation.None;
            ROCubeMetadatAttribute cubeMetadatAttributes = new ROCubeMetadatAttribute();

            ROCubeMetadata ROCubeMetadata = new ROCubeMetadata(returnCode, message, instanceID, gridOrientation, cubeMetadatAttributes);
                ...
                foreach (SimpleRowHeader rowHdrTag in rowHeaderList)
                {
                    ...
                    if (timeTotalProf != null)
                    {
                        if (timeTotalProf.VariableName != prevName)
                        {
                            ...
                            totalValues.ROCell[totalRowIndex][totalColumnIndex] = new ROCell(eCellDataType.Coordinate, displayName);
                        }
                    }
                    totalColumnIndex++;
                }
    ...
    /// Data that is defined per view
    /// </summary>
    public class ROStoreMultiLevelPeriodViewData : ROStoreMultiLevelViewData
    {
        public ROStoreMultiLevelPeriodViewData(int viewRID, ref ROPlanStoreMultiLevelManagerData managerData)
            : base(viewRID, ref managerData)
        {
            this.managerData = managerData;
        ...
        override public PagingCoordinates SetPageCoordinates(int iStartingRowIndex, int iNumberOfRows, int iStartingColIndex, int iNumberOfColumns)
        {
            ...
            SortedList workSortedTimeHeaders = CreateSortedList(_selectableTimeHeaders);
            _sortedTimeHeaders = new SortedList();
            first = (_startingRowIndex - 1) * _numberOfRows;
            last = first + _numberOfRows;
            ...
            bool selectTimePeriod = false;
            for (i = 0; i < workSortedTimeHeaders.Count; i++)
            {
                RowColProfileHeader rowColHeader = (RowColProfileHeader)workSortedTimeHeaders[i];
                ...
                if (selectTimePeriod)
                {
                    _sortedTimeHeaders.Add(rowColHeader.Sequence, rowColHeader);
                }
            }
